...
class Purchase_return extends CI_Controller {
...

    {
        ...
        ( SELECT *, ROW_NUMBER() OVER (ORDER BY id ASC) AS sl FROM purchase_return_tag WHERE  status!='' $fdate $tdate $uname )N 
...
// print_r(1);exit();
		...
		if(!empty($_FILES['add_party_new_loan']['name'])){
          ...
          $res = $this->db->query("INSERT INTO purchase_return_tag (
...
$res = $this->db->query("INSERT INTO purchase_return_tag_history(
    purchase_return_id,
    date,
    tag_id,
    img,
    ...
    status
        ...
        '".$tag_detail->tag_id."',
        '".$tag_detail->img."',
        '".$tag_detail->item_name."',
        '".$tag_detail->subitem_name."',
        '".$tag_detail->company_id."',
        '".$tag_detail->weight."',
        '".$tag_detail->stone."',
        '".$tag_detail->net_weight."',
...
// print_r( $data['tot_count']);exit;
        ...
        $res = $this->db->query("INSERT INTO purchase_return_nontag (
...
redirect('Purchase_return/nontag_return_history');
...

    {
        ...
        if($data['dt_fill']=="week"){
            ...
            // $data['nontag_return_list'] = $this->db->query("SELECT  *  FROM purchase_return_tag  ORDER BY id DESC ")->result_array();
            $data['nontag_return_list']  = $this->db->query(" SELECT  * FROM 
            ( SELECT *, ROW_NUMBER() OVER (ORDER BY id ASC) AS sl FROM purchase_return_nontag WHERE  status!='' $fdate $tdate $uname )N 
             WHERE      sl BETWEEN $offset AND $page_limit ")->result_array();
...
if(@getimagesize($image_url)){
										            	...
										            	$img='<div class="image-input" data-kt-image-input="true">
																			<div class="image-input-wrapper w-40px h-40px" style="background-image: url(<?php echo base_url() ?>tag_img/'. $view_list['img'].')"></div>
																		</div>';
    ...
    $img='<div class="image-input" data-kt-image-input="true">
            ...
            $rows.='<tr><td>'.$i.'</td><td>'.$company->COMPANYNAME.'</td><td>'.$date.'</td><td>'.$img.'</td><td>'.$item.'-'.$subitem_name->SUBITEM_NAME.'</td><td>'.$weight.'</td><td>'.$stone.'</td><td>'.$net_weight.'</td> </tr>';
          $i++; } 
...
$rows.='<tr><td>1</td> <td class="ple1">'. $company->COMPANYNAME.'</td><td>'. $tag_entry->tag_id.'</td>
<td>
'.$img_div.'
</td>
<td class="ple1">'. $item_name->ITEMNAME.'- '. $tag_entry->subitem_name.'</td><td>'. $tag_entry->weight.'</td><td>'. $tag_entry->stone.'</td><td><input type="hidden" name="net_weight[]" id=net_weight[] class="net_weight" value="'. $tag_entry->net_weight.'">'. $tag_entry->net_weight.'</td></tr>';
}
