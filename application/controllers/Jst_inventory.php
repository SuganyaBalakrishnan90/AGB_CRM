...
class Jst_inventory extends CI_Controller {
  ...
  $data['company_list']  = $this->db->query("SELECT * FROM COMPANY WHERE ACTIVE='Y'  ")->result_array();
...

    {
        ...
        $data['company_list']  = $this->db->query("SELECT * FROM COMPANY WHERE ACTIVE='Y'  ")->result_array();
        $data['old_gold_list']  = $this->db->query("SELECT * FROM salesentry WHERE oge_gold_count>0 OR oge_silver_count>0  ")->result_array();
        $data['staff_list']= $this->db->query("SELECT * FROM STAFFS   ")->result_array();  
    ...
    {
        $id=$_POST['id'];
        $tag_detail  = $this->db->query("SELECT * FROM tag_entry WHERE tag_id='".$id."'  ")->row();
...
$id='class="silver_weight";';
        ...
        $row='<tr class="tag_item"><input type="hidden" name="tag_id[]" value="'.$tag_detail->tag_id.'"><td>1</td><td>'.$tag_detail->tag_id.'</td><td '.$class.' >'.$jewel_type->jewel_type.'</td><td>'.$item_name->ITEMNAME.'-'.$subitem_name->SUBITEM_NAME.'</td><td '.$id.'>'.$tag_detail->net_weight.'</td><td>
        <a href="#" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm btnDelete" data-bs-toggle="modal" data-bs-target="#">
...
echo ''.'||'.$row.'||'.$tag_detail->item_name.'||'.$tag_detail->subitem_name.'||'.$tag_detail->metal_type.'||'.$tag_detail->net_weight;
...

    {
        ...
        // print_r($total_gold_weight);exit();
...
$jst_id=$prefix.str_pad($id,4,0,STR_PAD_LEFT).$suffix;
        ...
        for($i=0;$i<$tagged_count;$i++){
            ...
            $res = $this->db->query("INSERT INTO jst_inventory_tagged (
                tagged_id,
                jst_id,
                item_type,
                item,
          ...
          for($i=0;$i<$count;$i++){
            $old_metal_detail=$this->db->query("SELECT * from salesentry WHERE sales_id='".$checkbox[$i]."' ")->row();
        ...
        $res = $this->db->query("INSERT INTO jst_inventory (
			date,
			from_company,
			to_company,
			send_via,
...
add_notification(date('Y-m-d H:i:s'), $from_company, "Inventory", "New JST ", $jst_id. ' Created By ' .$_SESSION['username'],'', $_SESSION['USERID'], '0', $jst_id);
...

    {
        ...
        for($i=0;$i<$count1;$i++){
            ...
            $update1= $this->db->query("UPDATE jst_inventory_tagged SET status='M'  WHERE tagged_id= '".$checkbox1[$i]."' AND jst_id= '".$data['jst']->jst_id."' ");
    //   print_r("UPDATE jst_inventory_tagged SET status='M'  WHERE tagged_id= '".$checkbox1[$i]."' AND jst_id= '".$data['jst']->jst_id."' ");
        }
        for($i=0;$i<$count2;$i++){
            $update1= $this->db->query("UPDATE jst_inventory_old_metal SET status='M'  WHERE sales_id= '".$checkbox2[$i]."' AND jst_id= '".$data['jst']->jst_id."'");
        }
