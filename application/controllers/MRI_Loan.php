...
class Mri_loan extends CI_Controller {
   ...
   /*Mri LIst*/ 
    public function index()
    {
      ...
      $data['loan_list'] = $this->db->query("SELECT  * FROM ( SELECT (P.ADDRESS1 + ',' + P.ADDRESS2) AS ADDRESS,P.RATING, L.*, ROW_NUMBER() OVER (ORDER BY L.ENDATE DESC) AS sl FROM LOANS_MRI L  INNER JOIN PARTIES P ON P.PID=L.PID WHERE L.ACTIVE='Y' AND L.PID!='' $fdate $tdate $cmp)N WHERE sl BETWEEN $offset AND $page_limit  ORDER BY N.ENDATE DESC")->result_array();
...
/* MRI Add - Save data */
   public function mri_loan_save()
   {
    ...
    // exit();
    // echo "INSERT INTO LOANS_MRI (LOAN_SNO, NAME, BILLNO, LOAN_TYPE, ENDATE, PLEDGEINFO,JEWEL_TYPE, WEIGHT, STONELESS, LESS, NETWEIGHT, QUALITY, CURRENTRATE, PERGRAMVALUE,TOTALSALESRATE, AMOUNT, MONTHS, INTERESTTYPE, INTEREST, NETPAYABLE, MRI_AMOUNT, PAID_CASH,PAIDPRINCIPAL, PAIDINTEREST, BALANCE, LASTRECEIPTMONTH, NOMINEE, RELATION, REN_FROM_BILLNO,OLD_BILLNO, CCL_BILLNO, ACTIVE, STATUS, COMPANYCODE, ATTENDED_USER, ADDED_USER, ADDED_TIME, CHK_VERIFIED, REMARKS, VOUCHER_SNO, PID, LEDGER_SNO) VALUES ( '','".$loan_details->NAME."','".$mri_bill_no."','AUCTIONED','".$l_date."','".$loan_details->PLEDGEINFO."','".$loan_details->JEWEL_TYPE."','".$loan_details->WEIGHT."','".$loan_details->STONELESS."','".$loan_details->LESS."','".$loan_details->NETWEIGHT."','".$loan_details->QUALITY."','".$loan_details->CURRENTRATE."','".$loan_details->PERGRAMVALUE."','".$loan_details->TOTALSALESRATE."','".$loan_details->AMOUNT."','".$interest_list->PERIOD."','".$interest_list->INTNAME."','".$interest_list->INTEREST."','".$loan_details->AMOUNT."','".$int_paid."','0.00','0.00','".$int_paid."','".$loan_details->AMOUNT."','0','".$loan_details->NOMINEE."','".$loan_details->RELATION."','".$loan_details->BILLNO."','".$loan_details->BILLNO."','".$ccl_no."','Y','','".$cmp."','".$_SESSION['username']."','".$_SESSION['username']."','".date('Y-m-d H:i:s')."','N','".$desc."','','".$loan_details->PID."','')";
      // exit();

    $res=$this->db->query("INSERT INTO LOANS_MRI (LOAN_SNO, NAME, BILLNO, LOAN_TYPE, ENDATE, PLEDGEINFO,JEWEL_TYPE, WEIGHT, STONELESS, LESS, NETWEIGHT, QUALITY, CURRENTRATE, PERGRAMVALUE,TOTALSALESRATE, AMOUNT, MONTHS, INTERESTTYPE, INTEREST, NETPAYABLE, MRI_AMOUNT, PAID_CASH,PAIDPRINCIPAL, PAIDINTEREST, BALANCE, LASTRECEIPTMONTH, NOMINEE, RELATION, REN_FROM_BILLNO,OLD_BILLNO, CCL_BILLNO, ACTIVE, STATUS, COMPANYCODE, ATTENDED_USER, ADDED_USER, ADDED_TIME, CHK_VERIFIED, REMARKS, VOUCHER_SNO, PID, LEDGER_SNO) VALUES ( '','".$loan_details->NAME."','".$mri_bill_no."','AUCTIONED','".$l_date."','".$loan_details->PLEDGEINFO."','".$loan_details->JEWEL_TYPE."','".$loan_details->WEIGHT."','".$loan_details->STONELESS."','".$loan_details->LESS."','".$loan_details->NETWEIGHT."','".$loan_details->QUALITY."','".$loan_details->CURRENTRATE."','".$loan_details->PERGRAMVALUE."','".$loan_details->TOTALSALESRATE."','".$loan_details->AMOUNT."',".$interest_list->PERIOD.",'".$interest_list->INTNAME."',".$interest_list->INTEREST.",'".$loan_details->AMOUNT."','".$int_paid."','0.00','0.00','".$int_paid."','".$loan_details->AMOUNT."','0','".$loan_details->NOMINEE."','".$loan_details->RELATION."','".$loan_details->BILLNO."','".$loan_details->BILLNO."','".$ccl_no."','Y','','".$cmp."','".$_SESSION['username']."','".$_SESSION['username']."','".date('Y-m-d H:i:s')."','N','".$desc."','','".$loan_details->PID."','')");
    
    // "VALUES('" & txtReceiptNo.Text & "','" & UCase(Trim$(txtPName.Text)) & "','" & UCase(Trim$(txtBillno.Text)) & "','" & UCase(Trim$(cboLoanType.Text)) & "'," & _
    // "'" & Format(CDate(dtpEndate.Value), "dd/MMM/yyyy") & "','" & Left(GetPledgeName, 255) & "'," & _
    // "'" & Trim$(cboJewelType.Text) & "'," & Val(txtGrossWeight.Text) & "," & Val(txtStoneLess.Text) & "," & _
    // "" & Val(txtLess.Text) & "," & Val(txtNet.Text) & "," & Val(txtQuality.Text) & "," & _
    // "" & Val(txtCurrentRate.Text) & "," & Val(txtPerGramValue.Text) & ", " & Val(txtTotalSalesRate.Text) & "," & _
    // "" & Val(txtAmount.Text) & "," & Val(txtPeriod.Text) & ",'" & Trim$(cboIntTypes.Text) & "'," & _
    // "" & getInterestValue(Trim$(cboIntTypes.Text)) & "," & Val(txtNetPay.Text) & "," & Val(txtPaidAmount.Text) & "," & Val(txtMRIPaidAmount.Text) & ",0,0," & _
    // "" & Val(txtAmount.Text) & ",0,'" & Trim$(txtNominee.Text) & "','" & Trim$(cboRelations.Text) & "','" & Trim$(txtRenBillno.Text) & "','" & Trim$(txtOriginalLoanNo.Text) & "','" & Trim$(txtCclNo.Text) & "'," & _
    // "'Y','','" & vCompCode & "','" & Trim$(cboUsers.Text) & "','" & Trim$(ActiveUser) & "','" & Format(Now, "dd/MMM/yyyy hh:mm:ss AM/PM") & "','N','" & Trim$(txtAltRemarks.Text) & " ','" & vMasterSno & "','" & pid & "','" & ledgerPartyID & "')" ;
      redirect("mri_loan");
   }
      ...
      public function load_other_info()
      {
          ...
          $pid=$this->input->post("pid");
             ...
             if(substr($vIntType, 0,2)=="F-")
              ...
              $loan_stage=$this->db->Query("select * from loan_status where id=".$loan_details->LOAN_STATUS)->row();

              $loan_status='<span style="background-color:'.$loan_stage->color_code.';border-radius: 8px;padding: 5px 15px 5px 15px;">'.$loan_stage->loan_stage.'</span>';
              ...
              echo "||".$due_status."||".$receipt_count."||".$receipt_date."||".$days1."||".$months1."||".$loan_details->AMOUNT."||".$int."||".$pprinc."||".$pint."||".$rem_princ."||".$rem_int."||".$item_photo."||".$loan_status."||".$loan_details->WEIGHT."||".$loan_details->STONELESS."||".$loan_details->LESS."||".$loan_details->NETWEIGHT."||".$loan_details->CURRENTRATE."||".$loan_details->QUALITY."||".$loan_details->PERGRAMVALUE."||".$loan_details->TOTALSALESRATE."||".$loan_details->HL_AMOUNT;
      }
    ...
    {
      // $voucher_master_key = $this->Mri_Loan_model->get_voucher_master_key($_SESSION['LOANPREFIX']);
      // $vmkval = $voucher_master_key->KEYVALUE;
      // $vMasterSno = $data['vMasterSno'] = $_SESSION['LOANPREFIX'].($vmkval+1);

      // $account_payment_key = $this->Mri_Loan_model->get_account_payment_key($_SESSION['LOANPREFIX']);
