...
$rows='';
	   ...
	   $old_gold_list_detail=$this->db->query("SELECT * FROM salesentry_old_exchange WHERE sales_id='".$id ."' AND status='Y'")->result_array();
	   $i=1;
...
$rows.='<tr><td>'.$i.'</td><td>'.$date.'</td><td>'.$sales_id.'</td><td>'.$item_type->jewel_type.'</td><td>'.$img_div.'</td><td>'.$item_name->ITEMNAME.'</td><td>'.$subitem_name->SUBITEM_NAME.'</td><td>'.$weight.'</td><td>'.$st_weight.'</td><td>'.$less.'</td><td>'.$net_weight.'</td></tr>';
...
$rows.='<tr><td>'.$check_box.'</td><td>'.$date.'</td><td>'.$company->COMPANYNAME.'</td><td>'.$sales_id.'</td><td>'.$img_div.'</td><td>'.$subitem_name1.'</td><td>'.$weight.'</td><td>'.$st_weight.'</td><td>'.$less.'</td><td>'.$net_weight.'</td></tr>';

}

	   echo    ''.'||'.$item_type->jewel_type.'||'.$old_gold_wallet_list->ITEMNAME.'||'.$old_gold_wallet_list->OLD_GOLD_COUNT.'||'.$old_gold_wallet_list->OLD_GOLD_WEIGHT.'||'.$rows;
...
if($data['dt_fill']=="week"){
    ...
    //  print_r($data['week_to_date_fillter']);exit;
     ...
     $tdate =" AND date<='".$data['week_to_date_fillter']."'";
		...
		$data['old_gold_lot_list']=$this->db->query("SELECT * FROM  old_gold_lot WHERE status!='N' $fdate $tdate  $sname ")->result_array();
...
$i=1;
	...
	$img_div='<div class="image-input" data-kt-image-input="true">
			...
			$rows.='<tr><td>'.$i.'</td><td>'.$date.'</td><td>'.$company.'</td><td>'.$generate_no.'</td><td>'.$img_div.'</td><td>'.$item_name->ITEMNAME.'</td> <td>'.$subitem_name->SUBITEM_NAME.'</td><td>'.$weight.'</td><td>'.$stone.'</td><td>'.$less.'</td><td>'.$net_weight.'</td></tr>';
		$i++;
	}
...
$rows.='<tr><td>'.$i.'</td><td>'.$list_date.'</td><td>'.$list_generate_no.'</td><td>'.$img_div.'</td><td>'.$item_name->ITEMNAME.'</td><td>'.$subitem_name->SUBITEM_NAME.'</td><td>'.$list_weight.'</td><td>'.$list_stone.'</td><td>'.$list_less.'</td><td>'.$list_net_weight.'</td></tr>';
 $i++; }
