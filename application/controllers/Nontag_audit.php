...
***************************************************************************************/
class Nontag_audit extends CI_Controller {
...

    {
        ...
        $this->load->view('nontag_audit/non_tagged_audit',$data);
...
if(($item_list->COUNT==$count)&&($item_list->WEIGHT-0.1<=$weight)&&($item_list->WEIGHT+0.1>=$weight)){

    $res = $this->db->query("INSERT INTO nontag_audit_history (
...
add_notification(date('Y-m-d H:i:s'), '', "Inventory", "New Non Tag Audit ", ' Created By ' .$_SESSION['username'],'', $_SESSION['USERID'], '0', '-');
    ...
    public function update_description()
    {
        ...
        $get_id= $this->db->query("SELECT  *  FROM nontag_audit_history  ORDER BY id DESC ")->row();
        // print_r($get_id->id);exit;
        $res=$this->db->query("UPDATE nontag_audit_history SET description='".$description."'  WHERE id = '".$get_id->id."'");
        redirect('Nontag_audit/audit_history');
    }
...
if($data['dt_fill']=="week"){
    ...
    //  print_r($data['week_to_date_fillter']);exit;
     ...
     $tdate =" AND date<='".$data['week_to_date_fillter']."'";
        ...
        // $data['audit_history_list'] = $this->db->query("SELECT * FROM nontag_audit_history WHERE status!='' $fdate $tdate  $uname")->result_array();
        $data['audit_history_list']  = $this->db->query(" SELECT  * FROM 
          ( SELECT *, ROW_NUMBER() OVER (ORDER BY id ASC) AS sl FROM nontag_audit_history WHERE  status!='' $fdate $tdate $uname )N 
           WHERE      sl BETWEEN $offset AND $page_limit ")->result_array();
        $data['count']=count( $this->db->query("SELECT * FROM nontag_audit_history WHERE status!='' $fdate $tdate  $uname")->result_array());
       
    
        $this->load->view('nontag_audit/non_tagged_audit_history',$data);
       
    }


    public function nontag_audit_view(){
        
        $id=$_POST['id'];
        $nontag_audit_view = $this->db->query("SELECT * FROM nontag_audit_history WHERE id='".$id."'")->row();
        $item_name  = $this->db->query("SELECT * FROM ITEMS WHERE  SNO='". $nontag_audit_view->item."' ")->row();
        if($nontag_audit_view->description!=''){
           $description= $nontag_audit_view->description;
        }
        else{
            $description='-';
        }

        echo    ''.'||'.$item_name->ITEMNAME.'||'.$nontag_audit_view->count.'||'.$nontag_audit_view->weight.'||'.$description.'||'.$nontag_audit_view->id;
    }
    public function update_resolve()
    {
        ...
        $res=$this->db->query("UPDATE nontag_audit_history SET resolve='".$resolve."',status='R'  WHERE id = '".$id."'");
    add_notification(date('Y-m-d H:i:s'), '', "Inventory", "Update resolve Non Tag Audit ",  ' Updated By ' .$_SESSION['username'],'', $_SESSION['USERID'], '0','');
