        ...
        if(count($rows)>0) {
          foreach($rows as $row )
          {
             ...
             // }
              ...
              $title = $bill_no;
              $res.='{ "title":"'.$title.'","pid": "'.$party_id.'","firstname":"'.$party.'","rating":"'.$rating.'","address":"'.$address.'","inom_det":"'.$inom_det.'","nom_det":"'.$nom_det.'","weight":"'.$weight.'","pl_count":"'.$pl_count.'","tfoot":"'.$tfoot.'","tbody":"'.$tbody.'","loan_dt":"'.$loan_dt.'","expiry_dt":"'.$expiry_dt.'","company":"'.$company.'","loan_amt":"'.$loan_amt.'","interest":"'.$interest.'","phone":"'.$phone.'","item_photo":"'.$item_photo.'","party_photo":"'.$party_photo.'"},';
     ...
     public function nomineeList()
    {
     ...
     public function load_other_info()
        ...
        public function sd_new_view($id)
      {
        ...
        $data['pled'] = $this->db->query("SELECT * FROM temp_pledge_info WHERE BILLNO = '".$loan_no."' ")->result_array();
