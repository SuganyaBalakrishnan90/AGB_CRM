...
$this->db->reconnect();
    $data['Lotentry_list']  = $this->db->query("SELECT * FROM  ( SELECT *, ROW_NUMBER() OVER (ORDER BY lot_id DESC) AS sl FROM lot_entry )L WHERE status='A' $cname $sname $fdate $tdate  order by L.lot_no desc")->result_array();
    ...
    if($today=="Sunday"){
     ...
     $tdate =" AND date<='".$data['week_to_date_fillter']."'";
       ...
       // print_r($page);exit;
        ...
        $data['count'] = count($data['nontag_history']= $this->db->query("SELECT * FROM nontag_history  WHERE item_id='".$id."' $fdate $tdate $cname")->result_array());
        $data['company_list']  = $this->db->query("SELECT * FROM COMPANY WHERE ACTIVE='Y'  ")->result_array();
