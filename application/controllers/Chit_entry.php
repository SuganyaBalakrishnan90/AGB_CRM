...
/* ************************************************************************************
        Purpose : To handle all Chit_entry functions 2022-08-18
***************************************************************************************/
class Chit_entry extends CI_Controller {
    ...
    /* ************************************************************************************
                        Purpose : To handle Chit_entry List function 
            **********************************************************************/
    ...
    public function chitlist_transaction_history()
    {
      ...
      $data['count'] = count($this->db->query("SELECT PARTIES.NAME, PARTIES.RATING, PARTIES.PID, PARTIES.MEMBERSHIP_POINTS,CHIT_TRANSACTION.status, CHIT_TRANSACTION.party_id, CHIT_TRANSACTION.trans_date,CHIT_TRANSACTION.scheme_type, CHIT_TRANSACTION.scheme_id, CHIT_TRANSACTION.transaction_type, CHIT_TRANSACTION.opening_amount, CHIT_TRANSACTION.processing_amount, CHIT_TRANSACTION.closing_balance, CHIT_TRANSACTION.amt_paid ,
      CHIT_TRANSACTION.payment_id, CHIT_TRANSACTION.type
      FROM PARTIES
      INNER JOIN CHIT_TRANSACTION 
      ON PARTIES.PID = CHIT_TRANSACTION.party_id 
      WHERE CHIT_TRANSACTION.status = 'Y' $fdate $tdate $party_fil $scheme_sel $sta_sel ")->result_array());
      $data['chit_all_trans_list'] = $this->Chit_model->get_all_transaction_list( $party_fil,$fdate,$tdate,$scheme_sel,$sta_sel,$offset,$page_limit);
    ...
    public function chit_list_modal()
    {
        ...
        $data = $this->Chit_model->get_chit_modal_list($party_id);
         $arr  = [];
            foreach($data as $val)
            {
               $arr[] = [ 'chit_id' =>  $val['chit_id'],
                      'chit_date' => date("d-m-Y",strtotime($val['chit_date'])),
        ...
        $data = $this->Chit_model->get_chit_modal_transaction($id);
         $arr  = [];
            foreach($data as $val)
            {
               ...
               $arr[] = [ 'type' =>  $val['type_of_payment'],
                      'amount' => $val['amount'],
                      'bank' => $bankname,
                      'cheque_no' => $val['cheque_ref_no'],
                      'remarks' => $val['remarks'],
                      'amt_type' => $val['payment_side'],
                ];
    ...
    public function chit_entry_save()
    {
        ...
        $c_date = $this->input->post('collec_date'); 
        if($c_date != '')
        {
            $data['collec_date'] = date("d",$c_date);
        }
        else{
            $data['collec_date'] = '';
        }
...
//   print_r( $tot_deposit);exit();
        ...
        { 
            ...
            // $sm_chit_count = $this->Chit_model->add_master_tm($data);
            // $sm_chit_count = $this->Chit_model->add_master_tmg($data);
        }
        $pid = [];
        $data['cash_val'] = $this->input->post('cash_val');
        ...
        $data['bank_val'] = $this->input->post('bank_val');
        ...
        $data['rtgs_val'] = $this->input->post('rtgs_val');
    ...
    public function deposit_chit()
    {
        ...
        if($result)
        {
            $this->session->set_flashdata('g_success', $data['chit_id'].' Chit Deposited successfully...');
        }else{
        ...
        $result = $this->Chit_model->dp_chit_trans($data);
        $pid = [];
        $data['cash_val'] = $this->input->post('cash_val');
        $data['cash_amt'] = $this->input->post('cash_amt');
        $data['cash_details'] = $this->input->post('cash_details');
    ...
    public function withdraw_chit()
    {
        ...
        if($result)
        {
            $this->session->set_flashdata('g_success', $data['chit_id'].' Chit Withdrawn successfully...');
        }else{
            $this->session->set_flashdata('g_err', 'Could not approve Withdraw Entry!');
        }
        $result = $this->Chit_model->wt_chit_trans($data);
        $pid = [];
        $data['cash_val'] = $this->input->post('cash_val');
        $data['cash_amt'] = $this->input->post('cash_amt');
        $data['cash_details'] = $this->input->post('cash_details');
    ...
    public function chit_list_det()
    {
      ...
      if(count($rows)>0) {
        foreach($rows as $row)
        {
            ...
            $title = $row->chit_id;
            $res.='{ "title":"'.$title.'","chit_id": "'.$row->chit_id.'","party_id": "'.$row->party_id.'","scheme_id": "' .$row->scheme_id.'","collection_type": "'.$row->collection_type.'","collection_day": "'.$row->collection_day.'","collection_date": "'.$row->collection_date.'","party_name": "'.$data->NAME.'", "address": "'.$data->ADDRESS1.$data->ADDRESS2.$data->CITY.'","mobile": "'.$data->PHONE.'","rating": "'.$data->RATING.'", "member_id": "'.$data->MEMBERSHIP_ID.'","member_points": "'.$data->MEMBERSHIP_POINTS.'","scheme_ty": "'.$st.'","tot_sel_trans": "'.$sel_ava_bal.'","tot_tha_cash_trans": "'.$tha_cash_ava_bal.'","tot_tha_gold_trans": "'.$tha_gold_ava_bal.'", "sm_cc": "'.$smcc.'","tm_cc": "'.$tmcc.'","cc": "'.$cc.'","sel_amt": "'.$c->sm_cash_ava_amt.'","tha_amt": "'.$c->tm_cash_ava_amt.'","cash": "'.$chit_list->ava_balance.'"},';
        }
        $res=rtrim($res,',');
        $res.=']';
      }
      ...
      public function userList()
      {
        ...
        if(count($rows)>0) {
          foreach($rows as $row )
          {
              $title='';
              $name='';
              $firstname=$row->NAME.'--'.$row->LASTNAME_PREFIX.','.$row->FATHERSNAME.'--'.$row->ADDRESS1.','.$row->ADDRESS2.','.$row->CITY.'--'.$row->PHONE;
              $lastname=$row->NAME;
              $address=$row->ADDRESS1.', '.$row->ADDRESS2.', '.$row->CITY;
              $member_id=$row->MEMBERSHIP_ID;
              $member_points=$row->MEMBERSHIP_POINTS;
