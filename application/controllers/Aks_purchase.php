...
***************************************************************************************/
class Aks_purchase extends CI_Controller {
	...
	public function __construct()
	{
    	...
    	$this->session->set_userdata('comtitle', 'Aks_purchase');
	}
...
// print_r(1);exit;
    ...
    public function purchase_view()
    {
        ...
        // $this->load->view('Aks_purchase/',$data);
    }
    ...
    public function add_cart_list()
    {
        ...
        <div class="image-input-wrapper w-40px h-40px" style="background-image: url('.$image_url.')"></div>
         ...
         <td><input type="hidden" name="prd_id_hidden[]" id="prd_id_hidden'.$count.'" value="'.$plid.'">'.$img_div.$prd_name.'</td>
         <td> <div><input type="text" name="prd_wgt[]" id="prd_wgt'.$count.'" onkeyup="cart_prd_cal('.$count.')" onfocusout="cart_prd_cal_total('.$count.')" onclick="select_value('.$count.')" value="0" class="form-control form-control-lg form-control-solid fs-7" style="width:90px;"></div></td>

         <td onkeyup="cart_prd_cal()" name="lb_prd_price1[]" id="lb_prd_price1'.$count.'" ><input type="text" class="form-control form-control-lg form-control-solid fs-7 d-inline-block" style="width:70px;" name="lb_prd_price[]" id="lb_prd_price'.$count.'" value="'.$prd_pur_price.'"  onkeyup="cart_prd_cal('.$count.')" onfocusout="cart_prd_cal_total('.$count.')">/'.$unit_price.'
   ...
   public function aks_new_purchase(){
    ...
    {   
           ...
           // $slid = $_POST['id'];
           // // $count = $_POST['count'];
           // $data['stk_hst'] = $this->Akspurchase_model->get_stk_hst_list($slid);
           
           // // $cur_qty    =$data['stk_hst']->PRD_CUR_QTY;

           // print_r($data['stk_hst']);exit;
        ...
        //    $data['cash_amt']       = $this->input->post('cashamount');
        ...
        // print_r($data['pur_cash']); exit;

        // $data['pur_cbank_name'] = $this->input->post('');
        // $data['pur_ref_no']     = $this->input->post('');
        // $data['pur_pay_detail'] = $this->input->post('cashdetail');
       ...
       // print_r($data['prd_id'][$i]);
          ...
          if($bal_qty> $max_stock){
            add_notification(date('Y-m-d H:i:s'), '', "Others", "Maximum Stock Alert", $data['crt_qty']->AKS_PRD_NAME. ' Maximum stock Reached' .$_SESSION['username'],'', $_SESSION['USERID'], '0', $data['crt_qty']->AKS_PRD_NAME);
       ...
       // exit;
        $result = $this->Akspurchase_model->purchase_entry($data);

        add_notification(date('Y-m-d H:i:s'), '', "Others", "New Sale", $pur_id. ' purchased By ' .$_SESSION['username'],'', $_SESSION['USERID'], '0', $pur_id);
