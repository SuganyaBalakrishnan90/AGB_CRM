...
***************************************************************************************/
class Realestate_property extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
    $this->load->model(array("Realestate_property_model"));
    	...
    	$this->session->set_userdata('comtitle', 'Realestate_property');
	}
    ...
    public function index()
    {
      ...
      $data['pro_por_list'] = $this->Realestate_property_model->get_por_list($pro_type,$ploat_type,$offset,$page_limit);


     $data['get_pro_type'] = $this->db->query("SELECT * FROM realestate_purchase WHERE property_status='Existing' AND status='Y' ")->result_array();

      // print_r($data['por_list']);


           $data['count'] = count($this->db->query("SELECT * FROM realestate_purchase WHERE property_status='Existing'AND status='Y'  $pro_type $ploat_type order by id desc")->result_array());
     ...
     public function userList()
      {
        ...
        if(count($rows)>0) {
          ...
          foreach($rows as $row )
          {
              ...
              $res.='{ "title":"'.$title.'","id": "'.$row->PID.'","firstname":"'.$firstname.'","lastname":"'.$lastname.'","rating":"'.$rating.'","party":"'.$party_photo.'","address":"'.$address.'","phone":"'.$phone.'"},';
   ...
   public function realestate_property_entry(){
        ...
        $data['vao_group'] = $this->db->query("SELECT * FROM VAO_GROUP ORDER BY SNO")->result_array();
    ...
    {   
        ...
        $data['porp_date']  = $this->input->post('prop_date');
         ...
         if($last_pid_detail){
            ...
            $last_data= $last_pid_detail? $last_pid_detail->property_id :0;
                ...
                $request =  request_num(((int)$result+1), 3, "AREEP-");
       ...
       // print_r($data);exit();
        ...
        for($i=0;$i<10;$i++){
          ...
          if($data['ag_name'][$i]!=''){
            // print_r($ag_name);
              $res = $this->db->query("INSERT INTO realestate_agents
               (property_id
               ,agent_name
               ,amount
               ,Refname)
...
// print_r($data);exit();
     ...
     public function image_view()
    {
     ...
     public function realestate_property_edit($eid){

        $data['edit_details'] = $this->db->query("SELECT * FROM realestate_purchase WHERE id='".$eid."' ")->row();

        $get_party_id = $data['edit_details']->party_id;
        $get_prop_id = $data['edit_details']->property_id;
...
public function property_edit_save(){
       ...
       // print_r($data);exit();
        ...
        if($result)
        {
            $this->session->set_flashdata('g_success', $data['property_ids'].' &nbsp; New Proprety have been Updated successfully...');
        }else{
           $this->session->set_flashdata('g_err', 'Could not add Property !');
        }
        redirect('Realestate_property/');
