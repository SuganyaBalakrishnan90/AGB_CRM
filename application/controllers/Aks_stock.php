...
***************************************************************************************/
class Aks_Stock extends CI_Controller {
	...
	public function __construct()
	{
    	...
    	$this->session->set_userdata('comtitle', 'Aks_Stock');
	}
...
//         
        ...
        if($data['dt_fill']=="week"){
           ...
           $data['count'] = count($this->db->query("SELECT * FROM AKS_STOCK WHERE prd_id='".$prd_ids."' $fdate $tdate  order by stk_id desc")->result_array());
          ...
          $cat_change = $cat_change.'<tr>
                            ...
                            <td class="d-flex align-items-center"> 
                              ...
                              <div class="image-input" data-kt-image-input="true">
                               <div class="image-input-wrapper w-40px h-40px me-3" style="background-image: url('.$image_urls.')"></div>
                              </div>
                              <div class="d-flex flex-column">
                                <a href="javascript:;" class="text-gray-800 text-hover-primary mb-1 ">'.$plist["AKS_PRD_NAME"].'</a>
                              </div>
                            ...
                            <td>'.$plist["PRD_CUR_QTY"].'</td>
                             ...
                             <td>
                                ...
                                <input type="text" name="adjust_stk[]" id="" class="form-control form-control-lg form-control-solid" placeholder="" value="0">
    ...
    public function stock_update()
    {
      ...
      $data['adjust_stk'] = $this->input->post('adjust_stk');
       // print_r($data['adjust_stk']);
       // print_r($data['prd_id']);exit();
        $data['modified_by']      = $_SESSION['username'];
        $data['modified_on']      = date('Y-m-d H:i:s');
        // print_r($data['adjust_stk']);exit;
          ...
          $stk_date = date("Y-m-d",strtotime($date));
           ...
           ,modified_on
            ...
            if($res)
        {
            $this->session->set_flashdata('g_success', 'Product Adjust successfully...');
        }else{
           $this->session->set_flashdata('g_err', 'Could not Adjust Product !');
        }
           // $result = $this->Aks_stock_model->update_stock($curt_stk,$prd_ids);
            $curt_stk = $stk_up_qty;
         // print_r($data);exit;
