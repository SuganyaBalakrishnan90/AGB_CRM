...
//footer values
        ...
        foreach ($data['loan_receipt_list'] as $lrlist) {
            ...
            $data['charges_tot']+=($lrlist['MAINTAINCHARGE']+$lrlist['NOTICECHARGE']+$lrlist['FORMCHARGE']);
            $data['hl_bal_tot'] += $lrlist['HL_AMOUNT'];
            $data['paid_amt_tot'] += $lrlist['PAIDAMOUNT'];
        }
    ...
    public function loanList()
      {
        ...
        if(count($rows)>0) {
          foreach($rows as $row )
          {
              ...
              $title = $bill_no;
              $res.='{ "title":"'.$title.'","pid": "'.$party_id.'","firstname":"'.$party.'","member_id":"'.$member_id.'","member_points":"'.$member_points.'","rating":"'.$rating.'","address":"'.$address.'","inom_det":"'.$inom_det.'","nom_det":"'.$nom_det.'","card_type":"'.$card_type.'","weight":"'.$weight.'","pl_count":"'.$pl_count.'","tfoot":"'.$tfoot.'","tbody":"'.$tbody.'","loan_dt":"'.$loan_dt.'","expiry_dt":"'.$expiry_dt.'","company":"'.$company.'","loan_amt":"'.$loan_amt.'","interest":"'.$interest.'","phone":"'.$phone.'","party_photo":"'.$party_photo.'"},';
          ...
          // $data['tfoot_str']=$tfoot_str;
             ...
             foreach($pay_res as $plist)
             {
              ...
              if($plist['type_of_payment']=='UPI')
              {
                  ...
                  <label class="col-lg-1 col-form-label fw-semibold fs-6">Cmy.Bank</label>
                  <label class="col-lg-2 col-form-label fw-bold fs-5" id="lbl_upi_ref_no">'.$plist['company_bank'].'</label>
                  <label class="col-lg-1 col-form-label fw-semibold fs-6">Details</label>
