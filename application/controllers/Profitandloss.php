...
/* ************************************************************************************
		Purpose : To handle all Profitandloss functions 2022-10-17
***************************************************************************************/
class Profitandloss extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model(array("Profitandloss_model","Accountsgroup_model"));
        ...
        $this->Profitandloss_model->other_db = $this->load->database($config_app,TRUE);
    	...
    	$this->session->set_userdata('comtitle', 'Profitandloss');
	}
    ...
    public function index()
    {
        ...
        $gold_op_stock = $this->Profitandloss_model->get_gold_opening_stock();
        ...
        $silver_op_stock = $this->Profitandloss_model->get_silver_opening_stock();
        ...
        {
            $inv_master_rate = $this->Profitandloss_model->get_last_invoice_master_by_item($gcs['ITEM_SNO'],$end_date);
            ...
            {
                $prod_item = $this->Profitandloss_model->get_product_by_item_no($gcs['ITEM_SNO']);
        ...
        {
            $inv_master_rate = $this->Profitandloss_model->get_last_invoice_master_by_item($scs['ITEM_SNO'],$end_date);
            if(count((array)$inv_master_rate)>0)
            {
                $prev_purchase_rate = $inv_master_rate->RATE;
            }
            ...
            {
                $prod_item = $this->Profitandloss_model->get_product_by_item_no($scs['ITEM_SNO']);
                if(count((array)$prod_item)>0 && !is_null($prod_item->OP_RATE))
                {
                    $prev_purchase_rate = $prod_item->OP_RATE;
                }
        ...
        //Income Ledgers
        $data['income_ledger_list'] = $income_ledger_list = $this->Profitandloss_model->get_income_group();
        /*if(count((array)$income_ledger_list)>0)
        {
            foreach($income_ledger_list as $ill)
            {
                $income_voucher_master_amount = $this->Profitandloss_model->get_income_voucher_master_amount_by_ledger_no($ill['LEDGER_SNO'],$start_date,$end_date);
        ...
        //Expense Ledgers
        $data['expense_ledger_list'] = $expense_ledger_list = $this->Profitandloss_model->get_expense_group();
        /*if(count((array)$expense_ledger_list)>0)
        {
            foreach($expense_ledger_list as $ill)
            {
                $expense_voucher_master_amount = $this->Profitandloss_model->get_expense_voucher_master_amount_by_ledger_no($ill['LEDGER_SNO'],$start_date,$end_date);
        ...
        /*$data['liabilities_group'] = $this->Profitandloss_model->get_liabilities_group();
        $data['assets_group'] = $this->Profitandloss_model->get_assets_group();*/

        $this->load->view('profitandloss/profitandloss_list',$data);
    }
