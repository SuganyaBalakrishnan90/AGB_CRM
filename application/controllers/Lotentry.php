...
$data['dt_fill'] = $this->input->post('dt_fill_lot');
$data['dt_fill_no'] = $this->input->post('dt_fill_lot_no');
$data['from_date_fillter'] = $this->input->post('from_date_fillter_lot');
$data['to_date_fillter'] = $this->input->post('to_date_fillter_lot');
$fdate ='';
$tdate ='';
...
$this->db->reconnect();
   // $data['Lotentry_list']  = $this->db->query("SELECT ROW_NUMBER() OVER (ORDER BY lot_no DESC) AS row_number,* FROM lot_entry WHERE status!='N' $sname $fdate $tdate $dt_fill_no order by lot_no desc  row_number BETWEEN {$offset} + 1 AND {$limit} + {$offset}")->result_array();
    ...
    $data['Lotentry_list1']  = $this->db->query("SELECT SUM(pure_metal_weight) as weight,SUM(amount_to_pay) as amount,COUNT(lot_id) as count FROM lot_entry WHERE status='A' $sname $cname $fdate $tdate $dt_fill_no ")->row();
