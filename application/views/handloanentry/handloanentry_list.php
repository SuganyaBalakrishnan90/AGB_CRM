<?php $this->load->view("common");?>
	...
	<body data-kt-name="metronic" id="kt_body" class="header-fixed header-tablet-and-mobile-fixed toolbar-enabled toolbar-fixed aside-enabled aside-fixed">
						...
						<div id="kt_toolbar_container" class="container-fluid d-flex align-items-center">
							...
							<div class="flex-grow-1 flex-shrink-0 me-5">
								...
								<div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
									<!--begin::Title-->
									<h1 class="d-flex align-items-center text-dark fw-bold my-1 fs-3">Hand Loan Entry List
									<!--begin::Separator-->
							...
							<div class="row gy-5 g-xl-8">
								...
								<div class="col-xxl-8">
									...
									<div class="card card-xxl-stretch mb-5 mb-xl-8">
				                        ...
				                        <?php } ?>
										<form action="<?php echo base_url();?>handloanentry" method="post">
										...
										<div class="card-body py-4">
											...
											<table class="table align-middle table-row-dashed table-striped table-hover fs-7 gy-1 gs-2" id="kt_datatable_dom_positioning">
												...
												<tbody class="text-gray-600 fw-semibold">
													<!--begin::Table row-->
													<?php $i=1; foreach($handloanentry_list as $dlist){
														$pledger = $this->Handloanentry_model->get_party_ledger_by_id($dlist['PID']);
													...
													<tr>
														...
														<td>
															<a href="javascript:;" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1" data-bs-toggle="modal" data-bs-target="#kt_modal_edithandloanentry" onclick="handloanentry_edit('<?php echo $dlist['HL_SNO'];?>')">
																			...
																			<a href="javascript:;" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm" data-bs-toggle="modal" data-bs-target="#kt_modal_delete_handloanentry" onclick="handloanentry_delete('<?php echo $dlist['HL_SNO'];?>')">
																				<span class="svg-icon svg-icon-3">
																					<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
				...
				<div class="modal-content rounded">
					...
					<div class="modal-body pt-0 pb-15 px-5 px-xl-20">
						...
						<div class="mb-7 text-center">
							<h1>On Account - Handloan Payments Entry</h1>
						</div>
						<!--end::Heading-->
						<form id="general_validate" style="width: 100%;" method="POST" action="<?php echo base_url(); ?>handloanentry/handloanentry_save" enctype="multipart/form-data" onsubmit="return handloanentry_validation();">
								<div class="row">
									<label class="col-lg-1 col-form-label required fw-semibold fs-6">Date</label>
									<div class="col-lg-2 fv-row">
										<div class="d-flex align-items-center">
											
											<!-- <input class="form-control form-control-solid ps-12" name="add_date_hand_loan_entry" placeholder="Date" id="kt_datepicker_add_hand_loan_entry_date" value="<?php //echo date("d m Y"); ?>"/> -->
											<input type="date" class="form-control form-control-solid" name="add_date_hand_loan_entrys" placeholder="Select Date" id="add_date_hand_loan_entrys" />
										</div>
										<div class="fv-plugins-message-container invalid-feedback" id="add_date_hand_loan_entrys_err"></div>
									</div>
									<!--end::Col-->
								...
								<div class="col-lg-6">
									...
									<div class="row">
										...
										<div class="col-lg-6 fv-row">
											<!--begin::Select-->
											<!-- <select class="form-select form-select-solid" name="add_paid_from_hand_loan_entry" data-control="select2" data-hide-search="true">	
									...
									<div class="d-flex align-items-center justify-content-center mt-4">
										...
										<button type="submit" class="btn btn-primary" id="save_add_hand_loan_entry">Save</button>
									</div>
								</div>
								<div class="col-lg-6">
									<div class="row">
		...
		<!--begin::Modal - edit company-->
		<div class="modal fade" id="kt_modal_edithandloanentry" tabindex="-1" aria-hidden="true">
...
<div class="modal fade" id="kt_modal_viewdept" tabindex="-1" aria-hidden="true">
		...
		<div class="modal fade" id="kt_modal_delete_handloanentry" tabindex="-1" aria-hidden="true">
...
function handloanentry_validation()
{
    var err = 0;
    var add_date_hand_loan_entrys = $('#add_date_hand_loan_entrys').val();
    var amount = $('#amount').val();
    var paid_from = $('#paid_from').val();

    if(add_date_hand_loan_entrys.trim()==''){
        $('#add_date_hand_loan_entrys_err').html('Date is required!');
        err++;
    }else{
        $('#add_date_hand_loan_entrys_err').html('');
    }
...
function party_autocomplete()
{
	$("#party_name").autocomplete({ 
            valueKey:'title',
            source:[{
            url: base+'handloanentry/get_party_list?type=PRO_MANA&query=%QUERY%',
            type:'remote',
            ajax:{
			...
			$.ajax({
			type: "POST",
			url: baseurl+'handloanentry/get_party_details',
			async: false,
			type: "POST",
			data: "id="+pid,
			dataType: "html",
...
function loan_autocomplete()
{
	$("#loan_no").autocomplete({ 
            valueKey:'title',
            source:[{
            url: base+'handloanentry/get_loan_party_list?type=PRO_MANA&query=%QUERY%',
            type:'remote',
            ajax:{
...
function paid_ledger_autocomplete()
{
	$("#paid_from").autocomplete({ 
            valueKey:'title',
            source:[{
            url: base+'handloanentry/get_handloanentry_paid_ledger?type=PRO_MANA&query=%QUERY%',
            type:'remote',
            ajax:{
...
function handloanentry_edit_validation()
{
    var err = 0;
    var add_date_hand_loan_entrys = $('#add_date_hand_loan_entrys_edit').val();
    var amount = $('#amount_edit').val();
    var paid_from = $('#paid_from_edit').val();

    if(add_date_hand_loan_entrys.trim()==''){
        $('#add_date_hand_loan_entrys_edit_err').html('Date is required!');
        err++;
    }else{
        $('#add_date_hand_loan_entrys_edit_err').html('');
    }
...
function handloanentry_edit(val){
var baseurl= $("#baseurl").val();
//alert(val);
$.ajax({
type: "POST",
url: baseurl+'handloanentry/handloanentry_edit',
async: false,
type: "POST",
data: "id="+val,
dataType: "html",
success: function(response)
{
$('#kt_modal_edithandloanentry').empty().append(response);
$('#kt_modal_edithandloanentry').addClass('show');
...
{
	var baseurl= $("#baseurl").val();
	$('#kt_modal_edithandloanentry').removeClass('show');
	//$("#kt_modal_update_role").css("display", "none");
	$('.modal-backdrop').removeClass('show');
	window.location.href = baseurl+'handloanentry';
}

function handloanentry_delete(val){
var baseurl= $("#baseurl").val();
//alert(val);
$.ajax({
type: "POST",
url: baseurl+'handloanentry/handloanentry_delete',
async: false,
type: "POST",
data: "id="+val,
dataType: "html",
success: function(response)
{
$('#kt_modal_delete_handloanentry').empty().append(response);
$('#kt_modal_delete_handloanentry').addClass('show');
//$("#kt_modal_delete_role").css("display", "block");
}
});
}

function removeHandLoanEntry(val)
{ 
var baseurl= $("#baseurl").val();
$.ajax({
type: "POST",
url: baseurl+'handloanentry/delete',
async: false,
data:"field="+val,
success: function(response)
{
...
{
	var baseurl= $("#baseurl").val();
	$('#kt_modal_delete_handloanentry').removeClass('show');
	$('.modal-backdrop').removeClass('show');
	window.location.href = baseurl+'handloanentry';
}	
