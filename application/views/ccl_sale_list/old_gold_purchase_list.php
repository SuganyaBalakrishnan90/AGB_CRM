...
<style>
  ...
  #dfn #c_nme {
	...
	<body data-kt-name="metronic" id="kt_body" class="header-fixed header-tablet-and-mobile-fixed toolbar-enabled toolbar-fixed aside-enabled aside-fixed">
						...
						<div id="kt_toolbar_container" class="container-fluid d-flex align-items-center">
							...
							<div class="flex-grow-1 flex-shrink-0 me-5">
								...
								<div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
									<!--begin::Title-->
									<h1 class="d-flex align-items-center text-dark fw-bold my-1 fs-3">Old Gold Purchase List
									<!--begin::Separator-->
													...
													<div class="row">
														<table id="kt_datatable_ccl_pending_list_table" class="table align-middle table-row-dashed table-striped table-hover fs-7 gy-1 gs-2 dataTable">
																...
																<tbody class="text-gray-600 fw-semibold">
																	<?php $i=0; foreach($ccl_list as $clist){ ?>
																	<tr>
																		<td><?php echo $i+1; ?></td>
																		<td><?php echo $clist['ccl_date']; ?></td>
																		<td class="ple1"><?php 
																				$comp_res=$this->db->query("select * from COMPANY where COMPANYCODE='".$clist['company_code']."'")->row();
																				echo $comp_res->COMPANYNAME;?></td>
																		<td class="ple1"><?php echo $clist['billno']; ?></td>
																		<td class="ple1">
																			<?php 
																					$party_res=$this->db->query("select NAME, RATING, PID from PARTIES where PID='".$clist['pid']."'")->row();
																				?>
																			<span>
																		...
																		<td><span class="me-3"><?php $pl_count=$this->db->query("select count(*) as cnt from PLEDGEINFO where BILLNO='".$clist['billno']."'")->row(); 
																				echo $pl_count->cnt; ?></span>
																					<?php 
																				$loan_res=$this->db->query("select * from LOANS where BILLNO='".$clist['billno']."'")->row();
																		...
																		<td class="ple1"><?php echo $clist['curr_sales_amount'] ?></td>
																		<td>
																			<?php if($clist['ccl_status']=='Request'){ ?>
																			...
																			<?php } ?>
																			<?php if($clist['ccl_status']=='Reject'){ ?>
																			<label class="col-form-label fw-semibold fs-7" id="dfn">
																				...
																				<a href="javascript:;" class="menu-link flex-stack align-items-center" onclick="show_desc(<?php echo $clist['ccl_sale_list_id']; ?>);">
																					<i class="fas fa-info-circle fs-6" style="color:black;" title="<?php echo $clist['description']; ?>"></i>
																				</a>
																			</label>
																		<?php } ?>
																		<?php if($clist['ccl_status']=='Accept'){ ?>
																		...
																		<td>
																			<span class="text-end">
																				<a href="#" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm" 	data-bs-toggle="modal" data-bs-target="#kt_modal_view_old_gold_purchase" onclick="return old_gold_purchase_view(<?php echo $clist['ccl_sale_list_id']; ?>);" >
			...
			<div class="modal-dialog modal-xl">
          	...
          	<div class="row">
						...
						<div class="row">
							...
							<label class="col-lg-2 col-form-label fw-bold fs-4">20000.00</label>
		...
		<div class="modal fade" id="kt_modal_view_old_gold_purchase" tabindex="-1" aria-hidden="true">
		...
		<script>
			function date_fill_nontag_wallet_history()
			{
				...
				else if (dt_fill_nontag_wallet_history == "custom_date")
				{
					today_dt_nontag_wallet_history.style.display = "none";
					monthly_dt_nontag_wallet_history.style.display = "none";
		...
		<script type="text/javascript">
			function old_gold_purchase_view(bno)
			{
					// var bill_no=bno;
					...
					$.ajax({
									type: "POST",
									url: baseurl+'ccl_sale_list/old_gold_purchase_view',
									async: false,
									type: "POST",
									data: "bno="+bno,
									dataType: "html",
									...
									{
												$('#kt_modal_view_old_gold_purchase').empty().append(response);
												$('#kt_modal_view_old_gold_purchase').addClass('show');
												$('#kt_modal_view_old_gold_purchase').show();
			...
			function close_old_gold_purchase_view_modal()
			{
					// $('#kt_modal_view_old_gold_purchase').empty()(response);
