						...
						<div id="kt_toolbar_container" class="container-fluid d-flex align-items-center">
							...
							<div class="flex-grow-1 flex-shrink-0 me-5">
								...
								<div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
									<!--begin::Title-->
									<h1 class="d-flex align-items-center text-dark fw-bold my-1 fs-3">Balance Sheet
									<!--begin::Separator-->
							...
							<div class="row gy-5 g-xl-8">
								...
								<div class="col-xxl-8">
									...
									<div class="card card-xxl-stretch mb-5 mb-xl-8">
				                        ...
				                        <?php } ?>
										<form action="<?php echo base_url();?>balancesheet" method="post">
										...
										<div class="card-body py-4">
													...
													<table class="table align-middle table-row-dashed table-striped table-hover fs-7 gy-1 gs-2">
														...
														<thead>
															...
															<tr class="text-start text-muted fw-bold fs-7 gs-0">
																...
																<th class="min-w-125px">Liabilities</th>
														...
														<tbody class="text-gray-600 fw-semibold">
															<?php $totDebits=0;$vGrossLTotal=0; foreach($liabilities_group as $lgp){
																$tot=0;
																$group_list = $this->Balancesheet_model->get_ledger_group_list_by_group_name($lgp['GROUP_NAME']);
																foreach($group_list as $glist)
																{
																	$vOpeningBalance = 0;
																	$vOpeningSide = '';
																	$acc_led_group = $this->Balancesheet_model->get_account_ledger_group($glist['LEDGER_SNO']);
																	...
																	$liabilities_ledger_credit_debit = $this->Balancesheet_model->get_liabilities_ledger_credit_debit($glist['LEDGER_SNO'],$start_date,$end_date);
																	if(is_null($liabilities_ledger_credit_debit->TOTALDEBITS))
																	{
																		$vTotDebit = 0;
																	}
																	else
																	{
																		$vTotDebit = $liabilities_ledger_credit_debit->TOTALDEBITS;
																	}

																	if(is_null($liabilities_ledger_credit_debit->TOTALCREDITS))
																	{
																		$vTotCredit = 0;
																	}
																	else
																	{
																		$vTotCredit = $liabilities_ledger_credit_debit->TOTALCREDITS;
																	}
																	...
																	$vTotBalance = $vTotCredit - $vTotDebit;
																...
																<tr>
																	<td><b><?php echo $lgp['GROUP_NAME'];?></b></td>
																	<td></td>
															...
															<?php $GetOpeningProfitLoss = 0;
																$credit_opbal = $this->Balancesheet_model->get_credit_opening_balance();
																$debit_opbal = $this->Balancesheet_model->get_debit_opening_balance();

																if(is_null($credit_opbal->copbal))
																{
																	$copbal = 0;
																}
																else
																{
																	$copbal = abs($credit_opbal->copbal);
																}

																if(is_null($debit_opbal->dopbal))
																{
																	$dopbal = 0;
																}
																else
																{
																	$dopbal = abs($debit_opbal->dopbal);
																}

																$vTotal = $copbal - $dopbal;

																$GetOpeningProfitLoss = $vTotal;
																$vOpeningProfitLoss = $GetOpeningProfitLoss;
																$vCurrentProfitLoss = $vNetProfit - $vNetLoss;

																$vOpeningBalance = 0;
																$vOpeningSide = '';
																$acc_led_group = $this->Balancesheet_model->get_account_ledger_group(0);
																if(count((array)$acc_led_group)>0)
																{
																	$vOpeningBalance = $acc_led_group->OP_BALANCE;
																	$vOpeningSide = $acc_led_group->OP_SIDE;
																...
																$liabilities_ledger_credit_debit = $this->Balancesheet_model->get_liabilities_ledger_credit_debit(0,$start_date,$end_date);
																if(is_null($liabilities_ledger_credit_debit->TOTALDEBITS))
																{
																	$vTotDebit = 0;
																}
																...
																$vTransferredProfitLoss = $mm;

																$vClosingProfitLoss = $vOpeningProfitLoss + $vTransferredProfitLoss + $vCurrentProfitLoss;

																if($vClosingProfitLoss>0)
																{?>
																	<tr>
																		<td><b>PROFIT & LOSS A/C</b></td>
																		<td></td>
																		<td align="right"><?php echo number_format($vClosingProfitLoss,2);?></td>
																	</tr>
																	<tr>
																		<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Opening Balance</td>
																		<td align="right"><?php echo number_format($vOpeningProfitLoss,2);?></td>
																		<td></td>
																	</tr>
																	<?php if($vNetProfit>0){?>															
																		<tr>
																			<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Current Period - (Net Profit)</td>
																			<td align="right"><?php echo number_format($vNetProfit,2);?></td>
																			<td></td>
																		</tr>
																	<?php }?>
																	<?php if($vNetLoss>0){?>															
																		<tr>
																			<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Current Period - (Net Loss)</td>
																			<td align="right"><?php echo number_format($vNetLoss,2);?></td>
																			<td></td>
																		</tr>
																	<?php }?>
																	<?php if($vTransferredProfitLoss!=0){?>	
																		<tr>
																			<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Less: Transferred</td>
																			<td align="right"><?php echo number_format($vTransferredProfitLoss,2);?></td>
																			<td></td>
																		</tr>
																	<?php }?>
																<?php $totDebits+=abs($vClosingProfitLoss);}

																$vGrossLTotal = $vGrossLTotal + $totDebits;

																$debopbal_qry = $this->Balancesheet_model->get_all_debit_opening_balance();
																$creopbal_qry = $this->Balancesheet_model->get_all_credit_opening_balance();

																if(is_null($debopbal_qry->debopbal))
																{
																	$debopbal = 0;
																}
																else
																{
																	$debopbal = $debopbal_qry->debopbal;
																}

																$vTotDebits = $debopbal + $vOpeningStockValue + $vOpeningStockValue2;

																if(is_null($creopbal_qry->creopbal))
																{
																	$creopbal = 0;
																}
																else
																{
																	$creopbal = $creopbal_qry->creopbal;
																}

																$vTotCredits = $creopbal;
																$vTotal = $vTotDebits - $vTotCredits;
																$GetDiffOpening = $vTotal;

																if($GetDiffOpening>0)
																{?>
																	<tr>
																		<td>&nbsp;&nbsp;&nbsp;Diff. in Opening Balances</td>
																		<td></td>
																		<td align="right"><?php echo number_format($GetDiffOpening,2);?></td>
																	</tr>
																<?php $vGrossLTotal = $vGrossLTotal + $GetDiffOpening;}
												...
												<div class="col-lg-6">
													...
													<table class="table align-middle table-row-dashed table-striped table-hover fs-7 gy-1 gs-2">
														...
														<thead>
															...
															<tr class="text-start text-muted fw-bold fs-7 gs-0">
																<!-- <th class="min-w-125px cy" style="width: 20%;">Company</th> -->
																<th class="min-w-125px">Assets</th>
																<th class="min-w-125px">Amount</th>
																<th class="min-w-125px">Total</th>
															</tr>
															<!--end::Table row-->
														...
														<tbody class="text-gray-600 fw-semibold">
															...
															<?php 
																...
																{
																	$gold_product_list = $this->Balancesheet_model->get_gold_product_list();
																	foreach($gold_product_list as $gplist){
																		$inv_master_rate = $this->Balancesheet_model->get_last_invoice_master_by_item($gplist['ITEM_SNO'],$end_date);
															            ...
															            {
															                $prod_item = $this->Balancesheet_model->get_product_by_item_no($gplist['ITEM_SNO']);
															            ...
															            $mm = $vClosingStockAmt = $gplist['STOCK_QTY']*$prev_purchase_rate;
																...
																<?php foreach($gold_product_list as $gplist){
																		$inv_master_rate = $this->Balancesheet_model->get_last_invoice_master_by_item($gplist['ITEM_SNO'],$end_date);
															            if(count((array)$inv_master_rate)>0)
															            {
															                $prev_purchase_rate = $inv_master_rate->RATE;
															            ...
															            if($balance_type=='greater_zero')
																		{
																			if($mm!=0)
																			{
																				...
																				?>
																					<tr>
																						<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $gplist['ITEM_NAME'].' - ', $gplist['STOCK_QTY'];?></td>
																						<td align="right"><?php echo number_format($mm,2);?></td>
																						<td></td>
																					</tr>
																				<?php }
																	...
																	$vClosingStockValue = $tot;
																}

																if($vClosingStockValue2>0)
																{
																	$silver_product_list = $this->Balancesheet_model->get_silver_product_list();
																	foreach($silver_product_list as $gplist){
																		$inv_master_rate = $this->Balancesheet_model->get_last_invoice_master_by_item($gplist['ITEM_SNO'],$end_date);
															            if(count((array)$inv_master_rate)>0)
															            {
															                $prev_purchase_rate = $inv_master_rate->RATE;
																...
																<?php foreach($silver_product_list as $gplist){
																		$inv_master_rate = $this->Balancesheet_model->get_last_invoice_master_by_item($gplist['ITEM_SNO'],$end_date);
															            if(count((array)$inv_master_rate)>0)
															            {
															                $prev_purchase_rate = $inv_master_rate->RATE;
																	...
																	$vClosingStockValue2 = $tot;
																}
																?>

																<?php foreach($assets_group as $lgp){
																$tot=0;
																$group_list = $this->Balancesheet_model->get_asset_ledger_group_list_by_group_name($lgp['GROUP_NAME']);
																foreach($group_list as $glist)
																{
																	$vOpeningBalance = 0;
																	$vOpeningSide = '';
															...
															<?php 
																$lpaccled = $this->Balancesheet_model->get_loan_party_account_ledger();
																$closbal = 0;foreach($lpaccled as $lpaled)
																{
																	$voucher_master_details = $this->Balancesheet_model->get_voucher_master_amount_by_ledger_no($lpaled['LEDGER_SNO'],$start_date,$end_date);
																	if(is_null($voucher_master_details->TOTBAL))
																	{
																		$totbal = 0;
																	}
																	else
																	{
																		$totbal = $voucher_master_details->TOTBAL;
																	}

																	$closbal= $closbal+$lpaled['OP_BALANCE']+$totbal;
																}
																$GetGroupAmount = $closbal;
															?>
															<tr>
																<td><b>LOAN PARTY(S)</b></td>
																<td></td>
																<td align="right"><?php echo number_format($GetGroupAmount,2);?></td>
															</tr>
															<?php $totCredits = $totCredits + $GetGroupAmount;?>
															...
															if($vClosingProfitLoss<0)
																{?>
																	...
																	<?php }?>
																<?php $totCredits+=abs($vClosingProfitLoss);}

																$vGrossTotal = $vGrossTotal + $totCredits;
																...
																{?>
																	<tr>
																		<td>&nbsp;&nbsp;&nbsp;Diff. in Opening Balances</td>
																		<td align="right"><?php echo number_format(abs($GetDiffOpening),2);?></td>
																		<td></td>
																	</tr>
																<?php $vGrossTotal = $vGrossTotal + abs($GetDiffOpening);}
											...
											<br>
											<div class="row">
												<div class="col-lg-6"><h2><b>Total Liabilities : <?php echo number_format($vGrossLTotal,2);?></b></h2></div>
												<div class="col-lg-6"><h2><b>Total Assets : <?php echo number_format($vGrossTotal,2);?></b></h2></div>
											</div>
										</div>
										<!--end::Card body-->
									</div>
				...
				<div class="modal-content rounded">
					...
					<div class="modal-body pt-0 pb-15 px-5 px-xl-20">
						...
						<div class="mb-7 text-center">
							<h1>Balance Sheet Entry [Spend Money,Expenses]</h1>
						</div>
						<!--end::Heading-->
											...
											<div class="form-group">
												<div id="mcontent10">
													<div class="row" id="mid0">
														...
														<div class="col-lg-3 fv-row">
															<input type="text" class="form-control mb-2 mb-md-0" placeholder="Enter Amount" name="add_amt_payments[]" id="add_amt_payments0" style="font-size: 17px !important;font-weight: 600 !important;" onkeyup="getBalance SheetAmount(0)" onkeypress="return isNumberKey(event,this);"/>
				...
				<div class="modal-content rounded">
					...
					<div class="modal-body pt-0 pb-15 px-5 px-xl-20">
						...
						<div class="mb-13 text-center">
							<h1 class="mb-3">Balance Sheet Details</h1>
						</div>
						<!--end::Heading-->
						<div class="row mb-6">
							<!--begin::Label-->
							...
							<div class="row mb-6">
								<!--begin::Label-->
												...
												<div class="col-lg-8 fv-row fv-plugins-icon-container">
													<input type="text" name="company" class="form-control form-control-lg form-control-solid" placeholder="Balance Sheet Name" Value="Accounts" disabled>
													<div class="fv-plugins-message-container invalid-feedback"></div>
												</div>
												<!--end::Col-->	
							</div>
...
var title = $('title').text() + ' | ' + 'Balance Sheet';
