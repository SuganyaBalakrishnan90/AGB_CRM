						...
						<div id="kt_toolbar_container" class="container-fluid d-flex align-items-center">
							...
							<div class="flex-grow-1 flex-shrink-0 me-5">
								...
								<div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
									<!--begin::Title-->
									<h1 class="d-flex align-items-center text-dark fw-bold my-1 fs-3">Department List
									<!--begin::Separator-->
								...
								<div class="card-header border-0 pt-6">
									...
									<div class="card-toolbar">
										...
										<div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
											...
											<!--end::Svg Icon-->New Department</button>
											<!--end::Add user-->
										</div>
										<!--end::Toolbar-->
										<!--begin::Group actions-->
													...
													<div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
														...
														<form id="kt_modal_export_users_form" class="form" action="#">
															...
															<div class="fv-row mb-10">
																<!--begin::Label-->
																<label class="fs-6 fw-semibold form-label mb-2">Select Department:</label>
																<!--end::Label-->
																<!--begin::Input-->
																<select name="role" data-control="select2" data-placeholder="Select Company" data-hide-search="true" class="form-select form-select-solid fw-bold">
																	<option></option>
																	<option value="Department Details">Department Details</option>
									...
									<table class="table align-middle table-row-dashed table-striped table-hover fs-6 gy-1 gs-2" id="kt_datatable_dom_positioning">
										...
										<thead>
											...
											<tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0">
												...
												<th class="min-w-125px" style="width: 60%;" >Department Name</th>
										...
										<tbody class="text-gray-600 fw-semibold">
											<!--begin::Table row-->
											<?php $i=1; foreach($department_list as $dlist){?>
											<tr>
												
												<td><?php echo $dlist['DEPARTMENTNAME'];?></td>
												<td>
													<label class="form-check form-switch form-check-custom form-check-solid">
															<!-- <input class="form-check-input" type="checkbox" value="1" checked="checked"> -->
															<input class="form-check-input w-35px h-20px" type="checkbox" <?php if($dlist['STATUS']==0){echo "checked";} ?> id="activeunactive_<?php echo $i;?>"  name="activeunactive_<?php echo $i;?>" onchange="activeunactive(<?php echo $dlist['DEPARTMENTID']; ?>,<?php echo $i; ?>)" value="<?php echo $dlist['STATUS'];?>">
													</label>
												</td>
												<!--begin::Action=-->
												<td>
													...
													<a href="javascript:;" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1" data-bs-toggle="modal" data-bs-target="#kt_modal_editdept" onclick="department_edit('<?php echo $dlist['DEPARTMENTID'];?>')">
													...
													<a href="javascript:;" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm" data-bs-toggle="modal" data-bs-target="#kt_modal_delete_department" onclick="department_delete('<?php echo $dlist['DEPARTMENTID'];?>')">
		...
		<div class="modal fade" id="kt_modal_adddept" tabindex="-1" aria-hidden="true">
			...
			<div class="modal-dialog modal-m">
				...
				<div class="modal-content rounded">
					...
					<!--end::Modal header-->
					<!--begin::Modal body-->
					<form name="department_add_form" id="department_add_form" method="POST" action="<?php echo base_url(); ?>department/department_save" enctype="multipart/form-data" onsubmit="return department_validation();">
						<div class="modal-body pt-0 pb-15 px-5 px-xl-20">
							<!--begin::Heading-->
							<div class="mb-13 text-center">
								<h1 class="mb-3">New Department</h1>
							</div>
								<div class="row mb-6">
									<!--begin::Label-->
													...
													<div class="col-lg-8 fv-row fv-plugins-icon-container">
														<input type="text" id="department" name="department" class="form-control form-control-lg form-control-solid" placeholder="Department Name" onkeyup="department_unique(this.value);">
														<div class="fv-plugins-message-container invalid-feedback" id="department_err"></div>
													</div>
													<!--end::Col-->	
								</div>
							<!--begin::Actions-->
						...
						<div class="row mb-6">
							...
							<div class="row mb-6">
								<!--begin::Label-->
												...
												<!--begin::Col-->
												<div class="col-lg-8 fv-row fv-plugins-icon-container">
													<input type="text" name="company" class="form-control form-control-lg form-control-solid" placeholder="Department Name" Value="Accounts" disabled>
...
var title = $('title').text() + ' | ' + 'Department';
    $(document).attr("title", title);

function department_edit(val){
var baseurl= $("#baseurl").val();
//alert(val);
$.ajax({
type: "POST",
url: baseurl+'department/department_edit',
async: false,
type: "POST",
data: "id="+val,
dataType: "html",
...
var berr=0;
function department_unique(val)
{
	var baseurl= $("#baseurl").val();
	$.ajax({
		type:"POST",
		url:baseurl+'department/department_unique',
		data:{'value':val},
		cache: false,
		dataType: "html",
			...
			if(result>0)
			{
				$('#department_err').html('Department already exist!');
				berr=1;
			}
			else
			{
...
function department_validation()
{
    ...
    if(department_name.trim()==''){
        $('#department_err').html('Enter Department!');
        err++;
    }else{
        //$('#department_err').html('');
        if(berr>0)
...
var berre=0;
function department_unique_edit(val,dcid)
{
	var baseurl= $("#baseurl").val();
	$.ajax({
		type:"POST",
		url:baseurl+'department/department_unique_edit',
		...
		dataType: "html",
			...
			if(result>0)
			{
				$('#department_edit_err').html('Department already exist!');
				berre=1;
			}
			else
			{
				$('#department_edit_err').html('');
				berre=0;
			}
		}
	});
}
function department_edit_validation()
{
	var erre = 0;
	var department = $('#department_edit').val();

    if(department.trim()==''){
        $('#department_edit_err').html('Enter Department Name!');
        erre++;
    }else{
        //$('#department_err').html('');
        if(berre>0)
...
function activeunactive(val,ival) {
	...
	$.ajax({
		type:"POST",
		url:baseurl+'department/department_active',
		data:datastring,
		cache: false,
		dataType: "html",
		success: function(result){
            ...
            setTimeout(function() {
	         window.location = baseurl+"department";
	        }, 3000);
		},
...
$.ajax({
type: "POST",
url: baseurl+'department/department_delete',
async: false,
type: "POST",
data: "id="+val,
dataType: "html",
success: function(response)
{
$('#kt_modal_delete_department').empty().append(response);
$('#kt_modal_delete_department').addClass('show');
...
{
	var baseurl= $("#baseurl").val();
	$('#kt_modal_delete_department').removeClass('show');
	$('.modal-backdrop').removeClass('show');
	window.location.href = baseurl+'department';
}
