	...
	<body data-kt-name="metronic" id="kt_body" class="header-fixed header-tablet-and-mobile-fixed toolbar-enabled toolbar-fixed aside-enabled aside-fixed">
											...
											<div class="row">
												...
												<div class="col-lg-4">
													<div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
														...
														<!-- filter form -->
														<form method="POST" autocomplete="off" action="<?php echo base_url(); ?>Chit_entry/" enctype="multipart/form-data">
															<div class="px-7 py-5" data-kt-user-table-filter="form">			
																<div class="mb-2">
																<label class="form-label fs-6 fw-semibold">Party</label>
																	<div class="col-lg-12 fv-row fv-plugins-icon-container">
																		<input type="text" name="party_fil" id="party_fil" class="form-control form-control-lg form-control-solid" placeholder="Party Name">
																	</div>
																</div>
														...
														<form method="POST" id="list_export1" action="<?php echo base_url(); ?>Chit_entry/chit_list_export">
														...
														<a href="<?php echo base_url();?>Chit_entry/chit_add" target="_blank">
											...
											<table id="kt_datatable_responsive" class="table align-middle table-striped table-hover fs-7 gy-1 gs-2">
												<thead>
												   <tr class="text-start text-muted fw-bold fs-7 gs-0">
	                                                    ...
	                                                    <th class="min-w-25px" colspan="2" rowspan="1"><span class="ms-18">Thangamagal</span></th>
														<th class="min-w-80px" >Tot no of Chit</th>
														<th class="min-w-150px" style="width: 20%;"><span class="text-end">Actions</span></th>
													</tr>
                                                  <tr class="text-start text-muted fw-bold fs-7 gs-0">
												...
												<tbody class="text-gray-600 fw-semibold">
													...
													<tr>
														...
														<span class="align-items-center"><i class="fa-solid fa-star fs-5" style="<?php 
															if($val['rating']==1) echo 'color:#50cd89;';
															if($val['rating']==2) echo 'color:#ffc700;';
															if($val['rating']==3) echo 'color:red;';
															if($val['rating']=='' || is_null($val['rating'])) echo 'color:#d2d4dc;';
															?>">
														</i>&nbsp;<?php echo $val['name'];?></span></td>
														<td><i class="fas fa-list-ol fs-5" title="Selvamagal Cash Chit Count"></i>&nbsp; <?php echo $val['sm_count']; ?> &emsp;<?php echo round($val['sm_ava_balance'],2); ?></td>
	                                                    <td><i class="fas fa-list-ol fs-5" title="Thangamagal Cash Chit Count"></i>&nbsp;<?php echo $val['tmc_count']; ?>&emsp;<?php echo round($val['tmc_ava_balance'],2); ?></td>
														<td><i class="fas fa-list-ol fs-5" title="Thangamagal Gold Chit Count"></i>&nbsp;<?php echo $val['tmg_count']; ?>&emsp;<?php echo round($val['tmg_ava_balance'],2); ?></td>
															<td><?php echo '&nbsp',$val['sm_count']+$val['tmc_count']+$val['tmg_count'].'&nbsp';?></td>
														<td>
															<span class="text-end">
																...
																<a href="javascript:;">
																<button type="button" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm" data-bs-toggle="modal" data-bs-target="#kt_deposit_chit_details" title="Deposit" onclick="list_view1('<?php echo $val['pid'];?>');">
																<i class="bi bi-piggy-bank-fill eyc"></i>
																</button>
																</a>
																<a href="javascript:;">
																<button type="button" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm" data-bs-toggle="modal" data-bs-target="#kt_withdraw_chit_details" title="Withdraw" onclick="list_view2('<?php echo $val['pid'];?>');">
																<i class="fa-solid fa-right-from-bracket eyc fs-4"></i>
																</button>
																</a>
																<a href="<?php echo base_url();?>Chit_entry/chit_transaction_history/<?php echo $val['pid']; ?>" target="_blank">
							...
							<table id="kt_datatable_responsive" class="table align-middle table-striped table-hover fs-7 gy-4 gs-2">
								<thead>
									<tr class="text-start text-muted fw-bold fs-7 gs-0">
										...
										<th class="min-w-75px">Deposit</th>
										<th class="min-w-75px">Withdraw</th>
							...
							<table id="kt_datatable_responsive" class="table align-middle table-striped table-hover fs-7 gy-4 gs-2">
								<thead>
									<tr class="text-start text-muted fw-bold fs-7 gs-0">
										...
										<th class="min-w-100px">Collection Type</th>
										<th class="min-w-100px">Collection Day</th>
										<th class="min-w-100px">Total Deposit</th>
										<th class="min-w-100px">Total Withdraw</th>
										<th class="min-w-100px">Available Balance</th>
		...
		<script>
			function list_view(val)
			{
				...
				$.ajax({
				type: "GET",
				url: baseurl+'Chit_entry/chit_list_modal',
				async: false,
				type: "GET",
				data: "party_id="+val,
				dataType: "json",
				...
				
				{
					...
					if(response.length > 0){
						...
						response.map((data,i)=>{
							...
							{
								closed = '';
								edit = "<a href='<?php echo base_url(); ?>Chit_entry/edit_chit/"+chitid+"' class='btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1' title='close'><i class='fa-solid fa-circle-xmark fs-4'></i></a>"
								remove = "<a href='<?php echo base_url(); ?>Chit_entry/chit_delete/"+chitid+"' class='btn btn-icon btn-bg-light btn-active-color-primary btn-sm' title='delete'><span class='svg-icon svg-icon-3'><svg width='24' height='24' viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'><path d='M5 9C5 8.44772 5.44772 8 6 8H18C18.5523 8 19 8.44772 19 9V18C19 19.6569 17.6569 21 16 21H8C6.34315 21 5 19.6569 5 18V9Z' fill='currentColor'></path><path opacity='0.5' d='M5 5C5 4.44772 5.44772 4 6 4H18C18.5523 4 19 4.44772 19 5V5C19 5.55228 18.5523 6 18 6H6C5.44772 6 5 5.55228 5 5V5Z' fill='currentColor'></path><path opacity='0.5' d='M9 4C9 3.44772 9.44772 3 10 3H14C14.5523 3 15 3.44772 15 4V4H9V4Z' fill='currentColor'></path></svg></span></a>"
							}
							// date = (format('DD-MM-YYYY',response[i].chit_date));
							chit_amount=response[i].chit_amount;
							
							list += "<tr><td>"+[i+1]+"</td><td class='ple1'><p id='chit_date'>"+response[i].chit_date+"</p></td><td><p id='chit_id'>"+chit_id+"&nbsp;"+closed+"</p></td><td><p id='scheme_type'>"+$scheme+"</p></td><td><p>"+chit_amount+"</p></td><td><p id='colection_type'>"+$collection+"</p></td><td id='coll_dt'> "+collection_date+"</td>	<td><p id='tot_deposit'>"+parseFloat(response[i].tot_deposit).toFixed(2) +"</p></td>	<td><p id='tot_withdraw'>"+parseFloat(total_withdraw).toFixed(2) +"</p></td><td><p id='ava_amt'>"+parseFloat(response[i].ava_balance).toFixed(2)+"</p></td><td>"+edit+"&nbsp;"+remove+"</td></tr>" 
				...
				// console.log(response)
				{
					...
					if(response.length > 0){
						...
						response.map((data,i)=>{
							...
							{
								closed = '';
								deposit = "<a target='_blank' href='<?php echo base_url(); ?>Chit_entry/chit_deposit/"+response[i].sno +"'><i class='bi bi-piggy-bank-fill eyc'></i></a>"
							}
							chit_amount=response[i].chit_amount;
							list += "<tr><td>"+[i+1]+"</td><td class='ple1'><p id='chit_date'>"+response[i].chit_date +"</p></td><td><p id='chit_id'>"+chit_id+"&nbsp;"+closed+"</p></td><td><p id='scheme_type'>"+$scheme+"</p></td><td><p>"+chit_amount+"</p></td><td><p id='colection_type'>"+$collection+"</p></td> <td><p id='colection_ty'>"+collection_date+"</p></td><td><p id='tot_deposit'>"+parseFloat(response[i].tot_deposit).toFixed(2) +"</p></td>	<td><p id='tot_withdraw'>"+parseFloat(total_withdraw).toFixed(2) +"</p></td>	<td><p id='ava_balance'>"+parseFloat(response[i].ava_balance).toFixed(2) +"</p></td><td>"+deposit+"</td></tr>" 
				...
				// console.log(response)
				{
					...
					if(response.length > 0){
						...
						response.map((data,i)=>{
							...
							{
								var withdraw="<a target = '_blank' href='<?php echo base_url(); ?>Chit_entry/chit_withdraw/"+response[i].sno +"'><i class='fa-solid fa-right-from-bracket eyc fs-4'></i></a>";
								var closed = '';
							...
							chit_amount=response[i].chit_amount;
							list += "<tr><td>"+[i+1]+"</td><td class='ple1'><p id='chit_date'>"+response[i].chit_date +"</p></td>	<td><p id='chit_id'>"+chit_id+"&nbsp;"+closed+"</p></td><td><p id='scheme_type'>"+$scheme+"</p></td><td><p id='chit_amount'>"+chit_amount+"</p></td><td><p id='colection_type'>"+$collection+"</p></td><td><p id='colection_ty'>"+collection_date+"</p></td><td><p id='tot_deposit'>"+parseFloat(response[i].tot_deposit).toFixed(2) +"</p></td>	<td><p id='tot_withdraw'>"+parseFloat(total_withdraw).toFixed(2) +"</p></td>	<td><p id='ava_balance'>"+parseFloat(response[i].ava_balance).toFixed(2) +"</p></td><td>"+withdraw+"</td></tr>" 
		...
		$(document).ready(function() {
			...
			$(".move_to").on("click", function () {
				value=$(this).val();
				// alert(value);
				$('#filter_form').attr('action', "<?php echo base_url(); ?>Jst?page="+value);
				$("#filter_form").submit();
				e.preventDefault();
			});
		});
