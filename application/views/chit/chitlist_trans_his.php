  ...
  .dataTables_scroll thead{
	...
	<body data-kt-name="metronic" id="kt_body" class="header-fixed header-tablet-and-mobile-fixed toolbar-enabled toolbar-fixed aside-enabled aside-fixed">
							...
							<div class="flex-grow-1 flex-shrink-0 me-5">
								...
								<div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
									...
									<label class="d-flex align-items-center text-primary fw-bold my-1 fs-5">
										...
										<!--<span>&emsp;All</span>-->
										<span>&emsp;<?php if($scheme_sel==''){ echo "All"; }else{ if($scheme_sel=='1' || $scheme_sel=='2'){ echo 'Cash'; } else { echo 'Gold'; } } ?></span>
									</label>
									<span class="h-20px border-gray-400 border-start ms-3 mx-2"></span>
									<label class="d-flex align-items-center text-primary fw-bold my-1 fs-5">
								...
								<div class="col-xxl-8">
									...
									<div class="card card-xxl-stretch mb-5 mb-xl-8">
												...
												<div class="col-lg-4">
													<div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
														...
														<div class="menu menu-sub menu-sub-dropdown w-300px w-md-325px" data-kt-menu="true">
															<form method="POST" autocomplete="off" action="<?php echo base_url(); ?>Chit_entry/chitlist_transaction_history" enctype="multipart/form-data">
															<div class="px-7 py-5" data-kt-user-table-filter="form">			
																<div class="mb-2">
																<label class="form-label fs-6 fw-semibold">Party</label>
																	<div class="col-lg-12 fv-row fv-plugins-icon-container">
																		<input type="text" name="party_fil" id="party_fil" class="form-control form-control-lg form-control-solid" placeholder="Party Name" oninput="this.value = this.value.replace(/[^a-zA-Z.]/g, '').replace(/(\..*)\./g, '$1');">
														...
														<form id="list_export1" method="POST" action="<?php echo base_url();?>Chit_entry/chit_transaction_export">
														...
														<!-- <button type="button" class="btn btn-sm btn-outline btn-outline-solid btn-outline-warning btn-active-light-primary me-3" data-bs-toggle="modal" data-bs-target="#">Export</button> -->
															...
															<!-- <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#kt_modal_add_new_chit">
															<span class="svg-icon svg-icon-2">
																<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
										...
										<div class="card-body py-4">
											<!--begin::Table-->
											<table class="table align-middle table-striped table-hover fs-7 gy-1 gs-2" id="kt_datatable_responsive">
												<thead>
												...
												<tbody class="text-gray-600 fw-semibold">
													...
													<tr>
														<td><?php echo $i; ?></td>
														<td><?php echo date("d-m-Y", strtotime($catlist['trans_date']));?></td>
														<td class="ple1">
														...
														<span class="align-items-center"><i class="fa-solid fa-star fs-5" style="<?php 
															if($catlist['RATING']==1) { echo 'color:#50cd89;'; }
															else if($catlist['RATING']==2) { echo 'color:#ffc700;'; }
															else if($catlist['RATING']==3) { echo 'color:red;'; }
															else if($catlist['RATING']=='' || is_null($catlist['RATING'])) { echo 'color:#d2d4dc;'; }
															else { echo ''; }
															?>"></i>&nbsp;<?php echo $catlist['NAME'];?></span></td>
														<td><?php echo $catlist['chit_id']; if($catlist['type']==2) { echo '&nbsp;&nbsp;<i class="fa-solid fa-circle-xmark fs-4 text-danger mt-2" title="closed"></i></td>'; } ?>
														<td><?php 
														...
														<td align="center"><span class="text-end">
																<a href="javascript:;" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm" onclick="trans_modal('<?php echo $catlist['payment_id'];?>')" data-bs-toggle="modal" data-bs-target="#kt_modal_chit_payment_mode" title="View" onclick="view('<?php echo $catlist['party_id'];?>')">
																<i class="bi bi-eye-fill eyc"></i>
																</a>
															<!--	<a href="<?php base_url();?>Chit_entry/chit_trans_history" target="_blank">
				...
				<div class="modal-content rounded">
					...
					<div class="modal-body pt-0 pb-5 px-1 px-xl-20">
						...
						<table class="table align-middle fs-7 gy-2 gs-2" id="view_chit_pyt_receive_frm_party">
							...
							<tbody class="text-gray-600 fw-bold fs-4" id="party_pay">
							</tbody>
						</table>
						<div class="row mt-4">
							<div class="col-lg-6 text-start">
								<label class="col-form-label fw-bold fs-3">Total Amount &emsp;</label>
								<label class="col-form-label fw-bold fs-3" id="total_amt1">9730.00</label>
							</div>
						</div>
						</div>
						...
						<table class="table align-middle fs-7 gy-2 gs-2" id="view_ln_dels_to_pay_frm_cmy">
							...
							<tbody class="text-gray-600 fw-bold fs-4" id="company_pay">
						  </tbody>
						</table>
						<div class="row mt-4">
							<div class="col-lg-6 text-start">
								<label class="col-form-label fw-bold fs-3">Total Amount &emsp;</label>
								<label class="col-form-label fw-bold fs-3" id="total_amt2">9730.00</label>
							</div>
						</div>
						</div>
		...
		function trans_modal(val)
		{
			...
			$.ajax({
				type: "GET",
				url: baseurl+'Chit_entry/chit_transaction_modal',
				async: false,
				type: "GET",
				data: "pay_id="+val,
				dataType: "json",
				success: function(response)
				{
					...
					if(response.length > 0){
						...
						response.map((data,i)=>{
							...
							if(response != '')
							{
								list += "<tr><td>"+response[i].type+"</td><td>"+response[i].amount +"</td><td>"+response[i].bank +"</td><td>"+response[i].cheque_no+"</td><td>"+response[i].remarks+"</td></tr>" 
							}
							else
							{
					...
					$("#total_amt1").html(total)
					$("#total_amt2").html(total)
					$('#party_pay').html(list)
		...
		$(document).ready(function() {
			...
			$(".move_to").on("click", function () {
				value=$(this).val();
				// alert(value);
				$('#filter_form').attr('action', "<?php echo base_url(); ?>Chit_entry/chitlist_transaction_history?page="+value);
				$("#filter_form").submit();
				e.preventDefault();
			});
		});
