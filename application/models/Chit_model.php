...
class Chit_model extends CI_Model
{
 ...
 /* ************************************************************************************
    ...
    public function get_chit_modal_list($party_id)
    {
            ...
            $this->db->reconnect();
            $chit_list = $this->db->query("SELECT * FROM CHIT_LIST where party_id = '".$party_id."' AND status = 'Y'")->result_array();
            save_query_in_log();
            $this->db->close();
            return $chit_list;
    }
    ...
    public function add_mpoint($data)
    {
        ...
        $m_point = $this->db->query("UPDATE PARTIES SET 
                    MEMBERSHIP_POINTS = MEMBERSHIP_POINTS+'".$data['m_points_ad']."', 
    ...
    public function dp_chit_entry($data)
    {
        ...
        $result = $this->db->query("UPDATE CHIT_LIST SET 
                tot_deposit = tot_deposit+".$data['sm_cash_box'].",
        ...
        if($data['tm_cash_box'] != '')
        {
            ...
            $result = $this->db->query("UPDATE CHIT_LIST SET                
            tot_deposit = tot_deposit+".$data['tm_cash_box'].",
        ...
        if($data['tm_gold_box'] != '')
        {
            ...
            tot_deposit = tot_deposit+".$data['net_gm_lab1'].",
            ava_balance = ava_balance+".$data['net_gm_lab1'].",
           modified_by='".$_SESSION['username'] ."',
            modified_on = '".$date_time."'
            WHERE party_id = '".$data['party_id_hidden']."' AND  chit_id = '".$data['chit_id']."' ");
    ...
    public function get_all_transaction_list($party_fil,$tdate,$fdate,$scheme_sel,$sta_sel,$offset,$page_limit)
    {
        $this->db->reconnect();
        ...
        WHERE CHIT_TRANSACTION.status = 'Y' $party_fil $fdate $tdate $scheme_sel $sta_sel)N WHERE sl BETWEEN $offset AND $page_limit ORDER BY N.sno DESC")->result_array();
        save_query_in_log();
        $this->db->close();
        return $result;
    }
    ...
    public function add_lotentry_master($data)
    {
        ...
        public function wt_chit_entry($data)
        {
            ...
            if($data['sm_cash_box'] != '')
            {
                ...
                $result = $this->db->query("UPDATE CHIT_LIST SET               
                tot_withdraw = tot_withdraw+".$data['sm_cash_box'].",
                ava_balance = ava_balance-".$data['sm_cash_box'].",
                modified_by='".$_SESSION['username'] ."',
                modified_on = '".$date_time."'
                WHERE party_id = '".$data['party_id_hidden']."' AND  chit_id = '".$data['chit_id']."' ");
            ...
            if($data['tm_cash_box'] != '')
            {
                ...
                $result = $this->db->query("UPDATE CHIT_LIST SET  
                tot_withdraw = tot_withdraw+".$data['tm_cash_box'].",
                ava_balance = ava_balance-".$data['tm_cash_box'].",
                modified_by='".$_SESSION['username'] ."',
                modified_on = '".$date_time."'
                WHERE party_id = '".$data['party_id_hidden']."' AND  chit_id = '".$data['chit_id']."' ");
            ...
            {
                $result = $this->db->query("UPDATE CHIT_LIST SET 
                tot_withdraw = tot_withdraw+".$data['tm_gold_box'].",
