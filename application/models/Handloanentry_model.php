...
/***************************************************************
    Purpose : To handle all the Hand Loan Entry database details 2022-08-19
****************************************************************/
class Handloanentry_model extends CI_Model
{
    public $other_db;
  function __construct()
    ...
    public function get_handloanentry_list($sdate,$edate,$sodr)
    {
        $this->db->reconnect();
        /*$result  = $this->db->query("SELECT * FROM VOUCHER_DETAILS WHERE VOUCHERTYPE = 'PAYMENT' AND DEBIT!='0.00' AND ROWNO=1 $sdate $edate $sodr")->result_array();*/
        //echo "SELECT * FROM HL_PAYMENTS WHERE COMPANYCODE='".$_SESSION['COMPANYCODE']."' $sdate $edate $sodr";
        $result  = $this->db->query("SELECT * FROM HL_PAYMENTS WHERE COMPANYCODE='".$_SESSION['COMPANYCODE']."' $sdate $edate $sodr")->result_array();
        save_query_in_log();
        $this->other_db->close();
        return $result;
    }
    ...
    public function get_handloanentry_paid_ledger($search)
    {
    ...
    public function get_account_handloanentry_key_value($lprefix)
    {
    ...
    {
        $this->db->reconnect();
        $result  = $this->db->query("INSERT INTO HL_PAYMENTS(HL_SNO,ENDATE,HL_TYPE,BILLNO,PID,AMOUNT,PAID_BY,REMARKS,HL_TRANS_SNO,VOUCHER_SNO,COMPANYCODE,ADDED_USER,ADDED_TIME) VALUES('".$data['HL_SNO']."','".$data['add_date_hand_loan_entrys']."','".$data['type']."','".$data['loan_no']."','".$data['party_id']."','".$data['amount']."','".$data['paid_from']."','".$data['description']."','".$data['HL_TRANS_NO']."','".$data['VOUCHER_MASTER_SNO']."','".$_SESSION['COMPANYCODE']."','".$_SESSION['username']."','".date('Y-m-d H:i:s')."')");
        save_query_in_log();
        $this->db->close();
        return $result;
    }
    ...
    {
        $this->other_db->reconnect();
        $result  = $this->other_db->query("INSERT INTO VOUCHER_MASTER(MASTER_SNO,DETAILS_SNO,TRANSDATE,AMOUNT,VOUCHER_TYPE,TRANSTYPE,DESCRIPTION,ADDED_USER,ADDED_TIME) VALUES('".$data['VOUCHER_MASTER_SNO']."','".$data['sno']."','".$data['add_date_hand_loan_entrys']."','".$data['amount']."','ON ACCOUNT','".$data['TRANSTYPE']."','".$data['description']."','".$_SESSION['username']."','".date('Y-m-d H:i:s')."')");
        save_query_in_log();
        $this->other_db->close();
        return $result;
    }
    ...
    {
        $this->other_db->reconnect();
        $result  = $this->other_db->query("INSERT INTO VOUCHER_DETAILS (VOUCHER_MASTER_SNO,VOUCHER_SNO,ENDATE,ROWNO,LEDGER_SNO,DEBIT,CREDIT,VOUCHERTYPE,TRANSTYPE)VALUES ('".$data['VOUCHER_MASTER_SNO']."','".$data['sno']."','".$data['add_date_hand_loan_entrys']."','1','".$data['party_id']."','".$data['amount']."','0.00','ON ACCOUNT','".$data['TRANSTYPE']."')");
        save_query_in_log();
        $this->other_db->close();
        return $result;
    }
    ...
    {
        $this->other_db->reconnect();
        $result  = $this->other_db->query("INSERT INTO VOUCHER_DETAILS (VOUCHER_MASTER_SNO,VOUCHER_SNO,ENDATE,ROWNO,LEDGER_SNO,DEBIT,CREDIT,VOUCHERTYPE,TRANSTYPE)VALUES ('".$data['VOUCHER_MASTER_SNO']."','".$data['sno']."','".$data['add_date_hand_loan_entrys']."','2','".$data['paid_from_id']."','0.00','".$data['amount']."','ON ACCOUNT','".$data['TRANSTYPE']."')");
        save_query_in_log();
        $this->other_db->close();
        return $result;
    }
    ...
    {
        $query = $this->db->query("SELECT * FROM HL_PAYMENTS WHERE HL_SNO='".$hlsno."'");
        $result = $query->row();
        return $result;
    }
    ...
    {
        $this->db->reconnect();
        $result  = $this->db->query("UPDATE HL_TRANS SET HL_DATE='".$data['add_date_hand_loan_entrys']."',HL_ENTRY_TYPE='4',HL_LOAN_TYPE='".$data['type']."',HL_PID='".$data['party_id']."',HL_BILLNO='".$data['loan_no']."',HL_DEBIT='".$data['amount']."',HL_CREDIT='0',HL_REMARKS='".$data['description']."' WHERE HL_TRANS_SNO='".$vHLTransNo."'");
        save_query_in_log();
        $this->db->close();
        return $result;
    }
    ...
    {
        $this->db->reconnect();
        $result  = $this->db->query("UPDATE HL_PAYMENTS SET ENDATE='".$data['add_date_hand_loan_entrys']."',HL_TYPE='".$data['type']."',BILLNO='".$data['loan_no']."',PID='".$data['party_id']."',AMOUNT='".$data['amount']."',PAID_BY='".$data['paid_from']."',REMARKS='".$data['description']."',HL_TRANS_SNO='".$vHLTransNo."' WHERE HL_SNO ='".$vHLPaymentNo."'");
        save_query_in_log();
        $this->db->close();
        return $result;
    }
    ...
    {
        $this->other_db->reconnect();
        $result  = $this->other_db->query("UPDATE VOUCHER_MASTER SET TRANSTYPE='".$data['TRANSTYPE']."',TRANSDATE='".$data['add_date_hand_loan_entrys']."',AMOUNT='".$data['amount']."',DESCRIPTION='".$data['description']."' WHERE MASTER_SNO='".$voucher_master_sno."'");
        save_query_in_log();
        $this->other_db->close();
        return $result;
    }
    ...
    {
        $this->db->reconnect();
        $result  = $this->db->query("DELETE FROM HL_PAYMENTS WHERE HL_SNO = '".$vmsno."'");
        save_query_in_log();
        $this->db->close();
        return $result;
    }
