...
public function update_property_entry($data){
 ...
 public function get_por_list($pro_type,$ploat_type,$offset,$page_limit)
    {
        $this->db->reconnect();
        ...
        $result = $this->db->query(" SELECT  * FROM  ( SELECT *, ROW_NUMBER() OVER (ORDER BY id DESC) AS sl FROM realestate_purchase WHERE  property_status='Existing' AND status='Y' $pro_type $ploat_type )N  WHERE  sl BETWEEN $offset AND $page_limit ")->result_array();
