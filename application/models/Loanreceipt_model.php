...
class Loanreceipt_model extends CI_Model
{
  ...
  {
     $query = $this->db->query("SELECT l.BILLNO,c.COMPANYNAME,l.LOAN_TYPE,l.ENDATE,l.AMOUNT, i.PERIOD, l.INTERESTTYPE,l.WEIGHT,l.STONELESS,l.LESS,l.NETWEIGHT, l.MONTHS,l.LOANDAYS,l.ITEM_PHOTO,l.NOMINEE,l.RELATION,l.NOMINEE_ID, p.PID, p.NAME,p.ADDRESS1,p.ADDRESS2,p.PHONE,p.CITY,p.MEMBERSHIP_ID,p.MEMBERSHIP_POINTS,p.RATING,p.PHOTO,p.OTHER_PHOTO from LOANS l,PARTIES p,COMPANY c,INTERESTLIST i WHERE l.PID=p.PID and i.INTNAME=l.INTERESTTYPE and l.COMPANYCODE=c.COMPANYCODE and l.LOAN_STATUS>=1 and l.ACTIVE='Y' and  l.BILLNO LIKE '%".$search.'%'."'  AND l.CLOSING_STATUS IS NULL AND l.CLOSEDATE IS NULL");
  ...
  public function get_loan_receipts_details($billno)
    ...
    public function get_paid_receipt_details($billno,$chkOD,$vLoanPeriod, $vLoanIntPercent)
    {
        ...
        if($chkOD==false)
        {
            foreach ($result as $res)
            {
                ...
                {
                    $paid_details.= "<tr><td>$vLoanLastDate</td><td>" . date_format(date_create($res['RECEIPT_DATE']),$_SESSION['DATE_PATTERN']) . " </td><td>0.0</td><td></td><td></td><td>0.00</td><td> " . ($res['PRINCIPAL']+ $res['INT_AMOUNT'])."</td></tr>";
                }
                else
                {
                    $next_edt1=date('Y-m-d',strtotime($next_edt.' +3 months'));
                    $next_edt=$next_edt1;
                    $paid_details.= "<tr><td>$next_edt</td><td>" . date_format(date_create($res['RECEIPT_DATE']),$_SESSION['DATE_PATTERN']) . " </td><td>0.0</td><td></td><td></td><td>0.00</td><td> " . ($res['PRINCIPAL']+ $res['INT_AMOUNT'])."</td></tr>";
                }
            }
        }
        else
        {
             ...
             foreach ($result as $res)
             {
                if($vLoanLastDate<$res['RECEIPT_DATE'])
                {
                    ...
                    $paid_details.= "<tr><td>$vLoanLastDate</td><td>" . date_format(date_create($res['RECEIPT_DATE']),$_SESSION['DATE_PATTERN']) ."</td><td>0.0</td><td>".$res['CALC_PRINCIPAL'] ."</td><td> ".$res['CALC_INTEREST']."</td><td>" . ($res['PRINCIPAL']+ $res['INT_AMOUNT'])." </td><td> ".$res['BALANCE']."</td></tr>";
                }
                else
                {
                    $next_edt1=date('Y-m-d',strtotime($next_edt.' +3 months'));
                    $next_edt=$next_edt1;
                    $paid_details.= "<tr><td>$next_edt</td><td>" . date_format(date_create($res['RECEIPT_DATE']),$_SESSION['DATE_PATTERN']) ."</td><td>0.0</td><td>".$res['CALC_PRINCIPAL'] ."</td><td> ".$res['CALC_INTEREST']."</td><td>" . ($res['PRINCIPAL']+ $res['INT_AMOUNT'])." </td><td> ".$res['BALANCE']."</td></tr>";
                }
     ...
     public function get_paid_receipt_details1($billno)
    {
        ...
        foreach ($result as $res)
        {
            ...
            $paid_details.= "<tr><td></td><td>" .date_format(date_create($res['RECEIPT_DATE']),$_SESSION['DATE_PATTERN']) . " </td><td>0.0</td><td></td><td></td><td> " . ($res['PRINCIPAL']+ $res['INT_AMOUNT'])."</td><td></td></tr>";
        }
        return $paid_details;
    }
