...
class Village_model extends CI_Model
{
  function __construct()
    ...
    public function village_unique($val)
    {   
        $this->db->reconnect();
        $result  = $this->db->query("SELECT * FROM VILLAGE WHERE VILLAGENAME = '".$val."'")->row();
        save_query_in_log();
        $this->db->close();
        return $result;
    }
    ...
    public function village_unique_edit($val,$dcid)
    {
        $this->db->reconnect();
        $result  = $this->db->query("SELECT * FROM VILLAGE WHERE VILLAGENAME = '".$val."' AND VILLAGEID != ".$dcid)->row();
        save_query_in_log();
        $this->db->close();
        return $result;
    }
