...
/***************************************************************
    Purpose : To handle all the Bankentry database details 2022-08-19
****************************************************************/
class Bankentry_model extends CI_Model
{
    public $other_db;
  function __construct()
    ...
    public function get_bankentry_list($sdate,$edate,$sodr)
    {
        ...
        $result  = $this->other_db->query("SELECT MASTER_SNO,TRANSDATE,AMOUNT,DESCRIPTION,COMPANYCODE,ADDED_USER,ADDED_TIME,VOUCHER_NO FROM VOUCHER_MASTER  WHERE COMPANYCODE='".$_SESSION['COMPANYCODE']."' $sdate $edate AND VOUCHER_TYPE='CONTRA' $sodr")->result_array();
        save_query_in_log();
        $this->other_db->close();
        return $result;
    }
    public function get_ledger_details_by_voucher_master_no($vmno)
    {
        ...
        $result  = $this->other_db->query("SELECT LEDGER_NAME,ROWNO FROM VOUCHER_MASTER,VOUCHER_DETAILS,ACC_LEDGERS WHERE VOUCHER_MASTER.MASTER_SNO=VOUCHER_DETAILS.VOUCHER_MASTER_SNO AND VOUCHER_MASTER.COMPANYCODE=ACC_LEDGERS.COMPANYCODE AND VOUCHER_DETAILS.LEDGER_SNO=ACC_LEDGERS.LEDGER_SNO AND VOUCHER_MASTER_SNO='".$vmno."' AND ROWNO=1 ORDER BY ROWNO")->result_array();
        save_query_in_log();
        $this->other_db->close();
        return $result;
    }
    public function get_paidby_ledger_details_by_voucher_master_no($vmno)
    {
        ...
        $result  = $this->other_db->query("SELECT LEDGER_NAME FROM VOUCHER_MASTER,VOUCHER_DETAILS,ACC_LEDGERS WHERE VOUCHER_MASTER.MASTER_SNO=VOUCHER_DETAILS.VOUCHER_MASTER_SNO AND VOUCHER_MASTER.COMPANYCODE=ACC_LEDGERS.COMPANYCODE AND VOUCHER_DETAILS.LEDGER_SNO=ACC_LEDGERS.LEDGER_SNO AND VOUCHER_MASTER_SNO='".$vmno."' AND ROWNO= 2 ORDER BY ROWNO")->result_array();
        save_query_in_log();
        $this->other_db->close();
        return $result;
    }

    public function get_bankentry_ledger($search,$showparty)
    {
        ...
        $query = $this->other_db->query("SELECT LEDGER_NAME,LEDGER_SNO,cast(OP_BALANCE as varchar) as OP_BALANCE,OP_SIDE FROM ACC_LEDGERS WHERE COMPANYCODE='".$_SESSION['COMPANYCODE']."' AND GROUP_ID IN('7','8','22') AND LEDGER_NAME LIKE '".$search."%' ORDER BY LEDGER_NAME");
        $result = $query->result();
        return $result;
    }

    public function get_bankentry_paid_ledger($search)
    {
    ...
    public function get_bankentry_key_value($lprefix)
    {
    ...
    public function get_account_bankentry_key_value($lprefix)
    {
        $kname = "ACC_TRANSFER-".$lprefix;
        $query = $this->other_db->query("SELECT * FROM KEYMASTER WHERE KEYNAME='".$kname."'");
        $result = $query->row();
        return $result;
    }
    ...
    public function voucher_debit_details_add($data)
    {
        ...
        $result  = $this->other_db->query("INSERT INTO VOUCHER_DETAILS (VOUCHER_SNO,VOUCHER_MASTER_SNO,ENDATE,ROWNO,LEDGER_SNO,DEBIT,CREDIT,VOUCHERTYPE,TRANSTYPE)VALUES ('".$data['VOUCHER_SNO']."','".$data['VOUCHER_MASTER_SNO']."','".$data['ENDATE']."','".$data['ROWNO']."','".$data['paid_account_led_id']."','".$data['add_tota_bankentrys']."','0.00','".$data['VOUCHERTYPE']."','".$data['TRANSTYPE']."')");
        save_query_in_log();
        $this->other_db->close();
        return $result;
    }
    ...
    {
        $this->other_db->reconnect();
        $result  = $this->other_db->query("INSERT INTO VOUCHER_MASTER (MASTER_SNO,DETAILS_SNO,VOUCHER_TYPE,TRANSDATE,AMOUNT,DESCRIPTION,COMPANYCODE,ADDED_USER,ADDED_TIME,TRANSTYPE)VALUES ('".$data['VOUCHER_MASTER_SNO']."','".$data['VOUCHER_SNO']."','".$data['VOUCHERTYPE']."','".$data['ENDATE']."','".$data['add_tota_bankentrys']."','".$data['description']."','".$_SESSION['COMPANYCODE']."','".$_SESSION['username']."','".date('Y-m-d H:i:s')."','".$data['TRANSTYPE']."')");
        save_query_in_log();
        $this->other_db->close();
        return $result;
    }
    ...
    {
        $this->other_db->reconnect();
        $result  = $this->other_db->query("UPDATE VOUCHER_MASTER SET VOUCHER_TYPE = '".$data['VOUCHERTYPE']."',TRANSDATE = '".$data['ENDATE']."',AMOUNT = '".$data['add_tota_bankentrys']."',DESCRIPTION = '".$data['description']."',COMPANYCODE = '".$_SESSION['COMPANYCODE']."',ADDED_USER = '".$_SESSION['username']."',MODIFIED_TIME = '".date('Y-m-d H:i:s')."',TRANSTYPE = '".$data['TRANSTYPE']."' WHERE MASTER_SNO = '".$data['VOUCHER_MASTER_SNO']."'");
        save_query_in_log();
        $this->other_db->close();
        return $result;
    }
