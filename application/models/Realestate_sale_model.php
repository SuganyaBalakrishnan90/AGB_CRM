...
class Realestate_sale_model extends CI_Model
{
  function __construct()
    ...
    public function get_pur_list($fdate,$tdate,$party_name,$pro_type,$ploat_type,$offset,$page_limit)
    {
        ...
        $result = $this->db->query(" SELECT  * FROM  ( SELECT *, ROW_NUMBER() OVER (ORDER BY id DESC) AS sl FROM realestate_sale WHERE  status='Y' $fdate $tdate $party_name  $pro_type $ploat_type )N  WHERE  sl BETWEEN $offset AND $page_limit ")->result_array();
