...
class City_model extends CI_Model
{
  function __construct()
    ...
    // To get city unique
    public function city_unique($val)
    {   
        $this->db->reconnect();
        $result  = $this->db->query("SELECT * FROM CITY WHERE CITYNAME = '".$val."'")->row();
        save_query_in_log();
        $this->db->close();
        return $result;
    }
    ...
    public function city_unique_edit($val,$dcid)
    {
        $this->db->reconnect();
        $result  = $this->db->query("SELECT * FROM CITY WHERE CITYNAME = '".$val."' AND CITYID != ".$dcid)->row();
        save_query_in_log();
        $this->db->close();
        return $result;
    }
