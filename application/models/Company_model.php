...
class Company_model extends CI_Model
{
	...
	public function company_save($data)
	{
		...
		$result = $this->db->query("INSERT INTO COMPANY(COMPANYCODE, COMPANYNAME, ADDRESS1, ADDRESS2, CITY, PINCODE, STATE, PHONE, EMAIL, PAN_NO, GST_NO, REGNO, COMPANYNAME2, ADDRESSLINE, LOGINCODE, BUSINESS_TYPE, ACTIVE) VALUES ('".$data['company_id']."','".$data['company_name']."','".$data['address1']."','".$data['address2']."','".$data['city']."','".$data['pincode']."','".$data['state']."','".$data['phone']."','".$data['email']."','".$data['pan_no']."','".$data['gst_no']."','".$data['reg_no']."','".$data['pcname']."','".$data['paddress']."','".$data['code']."',".$data['btype'].",'Y')");
	...
	{
		$result = $this->db->query("UPDATE COMPANY SET COMPANYNAME='".$data['company_name']."',ADDRESS1='".$data['address1']."',ADDRESS2='".$data['address2']."',CITY='".$data['city']."',PINCODE='".$data['pincode']."',STATE='".$data['state']."',PHONE='".$data['phone']."',EMAIL='".$data['email']."', PAN_NO='".$data['pan_no']."', GST_NO='".$data['gst_no']."', REGNO='".$data['reg_no']."', COMPANYNAME2='".$data['pcname']."',ADDRESSLINE='".$data['paddress']."',LOGINCODE='".$data['code']."',BUSINESS_TYPE='".$data['btype']."',COMPANY_LOGO='' WHERE COMPANYCODE='".$data['company_id']."'");
		save_query_in_log();
		return $result;
	}
    ...
    {
        $this->db->reconnect();
        $result  = $this->db->query("UPDATE COMPANY SET ACTIVE = 'N' WHERE COMPANYCODE = '".$uid."'");
        save_query_in_log();
        $this->db->close();
        return $result;
    }
    ...
    {   
        $this->db->reconnect();
        $result  = $this->db->query("SELECT * FROM COMPANY WHERE COMPANYNAME = '".$val."' ")->row();
        save_query_in_log();
        $this->db->close();
        return $result;
    }
