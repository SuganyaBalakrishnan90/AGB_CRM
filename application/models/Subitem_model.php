...
class Subitem_model extends CI_Model
{
  function __construct()
    ...
    {
        $this->db->reconnect();
        $result  = $this->db->query("INSERT INTO SUBITEM_LIST (SUB_ID,item_id,jewel_type_id,SUBITEM_NAME,CREATED_ON,CREATED_BY,STATUS)VALUES ('".$data['sno']."','".$data['item_type']."','".$data['itemname_sub']."','".$data['Subitemname_sub']."','".$data['created_on']."','".$data['created_by']."','".$data['status']."')");
        save_query_in_log();
        $this->db->close();
        return $result;
    }
